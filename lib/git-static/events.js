...
function pushEventHttp(ca, auth, cookieJWT, hostname, repositoryKey, callback) {
    ...
    async.series({
        ...
        for (const repositoryKey of (typeof repositories === 'string' ? [repositories] : repositories)) {
            async.each(!skipSelf ? servers : servers.filter((item) => {
                ...
                pushEventHttp(results.ca, results.auth, results.cookieJWT, server, repositoryKey, cb);
            }, (err) => {
                if (err) {
