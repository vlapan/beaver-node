...
function addSettings(root, settings) {
    for (const [settingKey, settingValue] of Object.entries(settings)) {
        if (settingKey === 'match') {
            for (const [matchKey, matchValue] of Object.entries(settingValue)) {
                ...
                addSettings(matchLocation, matchValue);
            }
        } else if (Array.isArray(settingValue)) {
            settingValue.forEach((item) => root._add(settingKey, item));
        } else {
            root._add(settingKey, settingValue);
        }
    }
}
...
function generateFile(route, routePath, callback) {
    ...
    const { ssl } = route.source;
    const { presets } = config.routing.ssl;
    const preset = presets[ssl];
    ...
    NginxConfFile.create(routePath, (err, conf) => {
        ...
        function makeHost(port, secure) {
            ...
            if (typeof route.source.nginx === 'object') {
                addSettings(nserver, route.source.nginx);
            }
            ...
            if (route.source.root) {
                ...
                for (const [staticKey, staticValue] of Object.entries(route.source.static)) {
                    ...
                    if (typeof staticValue.nginx === 'object') {
                        addSettings(staticLocation, staticValue.nginx);
                    }
        ...
        if (sslOn) {
            ...
            jobs.openssl = (callback) => {
                fs.readdirSync(`${certPath}`).forEach((file) => {
                    ...
                    sslUtil.checkCertificateExpiration(fs.readFileSync(filePath), function (err, expiry) {
