                ...
                'openssl generate certificates': ['generate root certificates', 'openssl check certificates', async () => {
                    ...
                    try {
                        ...
                        for (const item of results) {
                            if (typeof item === 'object' && (typeof first === 'undefined' || first > item.expirationDays)) {
                                first = item.expirationDays;
                            }
                        }
                        if (first) {
                            const expireBefore = 30;
                            const date = new Date(Date.now() + (1000 * 60 * 60 * 24 * (first - expireBefore)));
                            o.modules.notificator.add({
                                date: date,
                                key: 'regenerate',
                                message: `Some certificates will expire soon, regenerate your configuration!`,
                            });
                        }
                    } catch (e) {
                        console.error(e);
