			...
			target.forEach(function (item) {
				if (typeof item === 'string') {
					...
					if (!item.external) {
						...
						hosts.push('server ' + (item.lan.ip || item.wan.ip) + ':' + targetPort + ' weight=3 max_fails=1 fail_timeout=10s');
						counter++;
					} else if (item.external && !external) {
						var ip = config.locations[item.location].ext.web.ip;
						var backup = localLocation && secure ? '' : ' backup';
						var list = localLocation && secure ? hostsExternal : hosts;
						if (localLocation && secure) {
							hosts.push('server 127.0.0.1:81 weight=3 max_fails=1 fail_timeout=10s backup');
						}
						if (config._routes[ip]) {
							...
							routers.forEach(function (host) {
								...
								list.push('server ' + host.wan.ip + ':' + (secure ? 443 : 80) + ' weight=3 max_fails=1 fail_timeout=10s' + backup);
							});
						} else {
							list.push('server ' + ip + ':' + (secure ? 443 : 80) + ' weight=3 max_fails=1 fail_timeout=10s' + backup);
						}
						external = true;
						counter++;
					}
	...
	generate: function (callback) {
		...
		async.parallel({
			openssl: function (callback) {
				...
				var tasks = [];
				tasks.push(openssl.bind(null, certPath, 'default'));
