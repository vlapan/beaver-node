...
const { parseAcls, parseKey } = require('../../utils/structure');
...
async function generateFile(route, routePath) {
    ...
    if (source?.nginx?.allow) {
        const allowList = parseAcls(config.parser, new Set([source?.nginx?.allow].flat().filter(Boolean)));
        const max = Math.max.apply(undefined, allowList.map((x) => x.value.length));
        source.nginx.allow = allowList.map((x) => {
...
module.exports = {
    async generate(o) {
        ...
        Object.assign(jobs, {
            ...
            'generate maps config files': async () => {
                const mapPath = path.normalize(`${mapsPath}/beaver_xff_map.conf`);
                const result = new Map();
                const r = [
                    [['default', '$remote_addr;', '# if nothing found']],
                    [...parseKey(config.parser, '@routers', result, false)].map((x) => {
                        return [x[0], '$proxy_add_x_forwarded_for;', `# ${x[1]}`];
                    }),
                ].flat();
                const map = file`
