...
function generateFile(route, routePath, callback) {
	...
	NginxConfFile.create(routePath, function (err, conf) {
		...
		function makeHost(port, secure) {
			...
			nserver._add('listen', port + ' reuseport' + (isDefault ? ' default_server' : ''));
			...
			target.forEach(function (item) {
				if (typeof item === 'string') {
					...
					hosts.push('server ' + item.replace(/^https?:\/\//gi, '') + ' weight=3 max_fails=1 fail_timeout=' + fail_timeout);
					counter++;
				} else if (typeof item === 'object') {
					if (!item.external) {
						var targetPort = config.routing.types[item.type || 'unix'].level6[secure ? 'secure' : 'plain'];
						hosts.push('server ' + ((item.lan && item.lan.ip) || (item.wan && item.wan.ip)) + ':' + targetPort + ' weight=3 max_fails=1 fail_timeout=' + fail_timeout);
						counter++;
					} else if (item.external && !external) {
						var backup = localLocation && !secure ? ' backup' : '';
						var list = localLocation && secure ? hostsExternal : hosts;
						localLocation && secure && hosts.push('server 127.0.0.1:81 weight=3 max_fails=1 fail_timeout=' + fail_timeout + ' backup');
						if (localLocation && secure && item.wan) {
							list.push('server ' + item.wan.ip + ':443 weight=3 max_fails=1 fail_timeout=' + fail_timeout);
						} else {
							var location = config.locations[item.location];
							var ip = location && location.ext && location.ext.web && location.ext.web.ip;
							if (!ip) {
								return;
							}
							if (config._routes[ip]) {
								...
								routers.forEach(function (host) {
									...
									list.push('server ' + host.wan.ip + ':' + (secure ? 443 : 80) + ' weight=3 max_fails=1 fail_timeout=' + fail_timeout + '' + backup);
								});
							} else {
								list.push('server ' + ip + ':' + (secure ? 443 : 80) + ' weight=3 max_fails=1 fail_timeout=' + fail_timeout + '' + backup);
							}
						}
						external = true;
						counter++;
			...
			if (hostsExternal.length) {
				...
				nserver._add('listen', '81 reuseport');
...
module.exports = {
	generate: function (callback) {
		...
		if (!config.routing.ssl || !config.routing.ssl.defaultPreset || !config.routing.ssl.presets) {
			callback && callback('!config.routing.ssl || !config.routing.ssl.defaultPreset || !config.routing.ssl.presets');
			return;
		}
