    ...
    await new Promise(resolve => {
        NginxConfFile.create(routePath, (err, conf) => {
            ...
            function makeHost(port, secure) {
                ...
                const redirectHttps = !secure && (source.redirectHttp === true || source.proxyHttp === true);
                if (redirectHttps) {
                ...
                const headers = {};
                if (typeof source.nginx === 'object' && !redirectHttps) {
                ...
                } else if (source.static && !redirectHttps) {
                    backendLocationKey = backendExists ? '@backend' : '=404';
                    for (const [staticKey, staticValue] of Object.entries(source.static)) {
                        ...
                        if (staticValue.errorPage) {
                            ...
                            if (Array.isArray(staticValue.errorPage)) {
                                staticValue.errorPage.forEach((x) => errors.push(x));
        ...
        if (sslOn) {
            ...
            Object.assign(jobs, {
                'generate root certificates': async () => {
                    try {
                        ...
                        for (const item of expiryData) {
                            if (typeof first === 'undefined' || first > item.expireDate) {
                        ...
                        if (first) {
                            o.modules.notificator.add({
                                date: first,
                                key: 'regenerate-root',
                                message: `Some root certificates will expire soon! ${expiryData.map((x) => x.expiry.toISOString() + ' ' + x.key).join('\n')}`,
                            });
                        }
                    } catch (e) {
                        console.error(e);
                ...
                'openssl generate certificates': ['generate root certificates', 'openssl check certificates', async () => {
                    ...
                    try {
                        const results = await async.parallelLimit(tasks, 4);
                        ...
                        for (const item of results) {
                            ...
                            if (typeof first === 'undefined' || first.expirationDays > item.expirationDays) {
                                first = item;
                            }
                        }
                        expiryArr.sort((a, b) => {
                        ...
                        if (first) {
                            o.modules.notificator.add({
                                date: first.expireDate,
                                key: 'regenerate',
                                message: `Some certificates will expire soon, regenerate your configuration! ${expiryArr.map((x) => x.expiry.toISOString() + ' ' + x.key).join('\n')}`,
                            });
                        }
                    } catch (e) {
                        console.error(e);
