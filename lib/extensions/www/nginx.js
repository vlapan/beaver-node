        ...
        function makeHost(port, secure) {
            ...
            for (const item of target) {
                if (item.TargetStatic) {
                    ...
                    if (url) {
                        hosts.push(`server ${url.substring(url.indexOf('://') + 3)} weight=3 max_fails=${maxFails} fail_timeout=${failTimeout}`);
                        counter += 1;
                    }
                } else {
                    if (item.location === config.parser.location) {
                        const targetPort = config.routing.types[item.source.type || 'unix'].level6[secure ? 'secure' : 'plain'];
                        // console.log('#### ' + routeKey + ' ||| ' + item.lan3 + ' ||| ' + item.wan3 + ' ||| ' + route.hasLocalEndpoints);
            ...
            routeTest += `echo curl -I ${secure ? '-k ' : ''}${protocol}://${config._hostConfig.wan.ip} -H "Host: ${routeKey}"\n`;
            routeTest += `curl -I ${secure ? '-k ' : ''}${protocol}://${config._hostConfig.wan.ip} -H "Host: ${routeKey}"\n\n`;
            ...
            nlocation._add('proxy_pass', `${route.hasLocalEndpoints || !secure ? 'http' : 'https'}://${key}_${protocol}`);
    ...
    generate(callback) {
        ...
        if (sslOn) {
            ...
            jobs.openssl = (callback) => {
                openssl.generateRoot(certPath, (err) => {
                    ...
                    const keys = Object.keys(config._routes);
                    const tasks = [];
        ...
        jobs.conf = (callback) => {
            ...
            for (const target of config.parser.targets.list) {
                ...
                generateFile(target, routePath, (err, routeTest) => {
                    generatedCount += 1;
                    if (outputTest) {
                        outputTest += routeTest;
                    }
                    logger.log('info', `"${target.key}" finished, waiting for other ${config.parser.targets.list.length - generatedCount}`);
                    if (Object.keys(config.parser.targets.list).length - generatedCount === 0) {
            ...
            // const defaultRoutePath = path.normalize(`${siteEnabledPath}/default.conf`);
            // generateFile('default', defaultRoutePath, (err, routeTest) => {});

            // Object.keys(config._routes).forEach((route) => {
            //     const routePath = path.normalize(`${siteEnabledPath}/${route}.conf`);
            //     generateFile(route, routePath, (err, routeTest) => {
            //         generatedCount += 1;
            //         if (outputTest) {
            //             outputTest += routeTest;
            //         }
