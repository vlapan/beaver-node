    ...
    NginxConfFile.create(routePath, (err, conf) => {
        ...
        function makeHost(port, secure) {
            ...
            if (route.source.root) {
                ...
                staticLocation._add('try_files $uri @backend');
            } else if (route.source.static) {
                backendLocationKey = '@backend';
                for (const [key, value] of Object.entries(route.source.static)) {
                    nserver._add(`location ${key}`);
                    let staticLocation = nserver[`location ${key}`];
                    if (staticLocation.length) {
                        staticLocation = staticLocation[staticLocation.length - 1];
                    }
                    staticLocation._add('root', `${argv.home}/git-static/repositories/${value.repository}/${value.sourcePath}`);
                    staticLocation._add('try_files $uri @backend');
                }
            }
