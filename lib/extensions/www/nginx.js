                ...
                if (item.TargetStatic) {
                    ...
                    if (typeof redirect === 'string') {
                        ...
                        if (url === true) {
                            ...
                            let backup = '';
                            if (item.location && (item.location === config.parser.location) && route.hasLocalEndpoints && !secure && hosts.length > 1) {
                                backup = ' backup';
                            }
                            hosts.add(`server ${uri} weight=3 max_fails=${maxFails} fail_timeout=${failTimeout}${backup} # ${item.toString()}`);
                        }
                    }
                } else {
