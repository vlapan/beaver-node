...
function generateFile(route, routePath, callback) {
	...
	NginxConfFile.create(routePath, function (err, conf) {
		...
		function makeHost(port, secure) {
			...
			if (secure) {
				nserver._add('ssl', 'on');
				nserver._add('ssl_certificate', 'cert/' + route + '.chained.crt');
					...
					if (!item.external) {
						...
						if (config._routes[ip]) {
							...
							routers.forEach(function (host) {
								...
								hosts.push('server ' + host.wan.ip + ':' + (secure ? 443 : 80) + ' weight=3 max_fails=1 fail_timeout=2s' + backup);
							});
						} else {
							hosts.push('server ' + ip + ':' + (secure ? 443 : 80) + ' weight=3 max_fails=1 fail_timeout=2s' + backup);
						}
						external = true;
						counter++;
					}
