...
function generateFile(route, routePath, callback) {
	...
	NginxConfFile.create(routePath, function (err, conf) {
		...
		function makeHost(port, secure) {
			...
			if (secure) {
				nserver._add('ssl', 'on');
				nserver._add('ssl_certificate', 'cert/host-' + route + '.crt');
				nserver._add('ssl_certificate_key', 'cert/host-' + route + '.key');
			}
				...
				if (typeof item === 'string') {
					...
					if (!item.external) {
						...
						if (localLocation && secure && item.wan) {
							...
							var location = config.parser.locations.map[item.location];
							if (location && location.wan3) {
								[].concat(location.wan3).forEach(function (item) {
