			...
			target.forEach(function (item) {
				if (typeof item === 'string') {
					...
					if (!item.external) {
						...
						var backup = config.routing.routes[route].localLocation ? ' backup' : '';
						if (config._routes[ip]) {
							...
							routers.forEach(function (host) {
								...
								hosts.push('server ' + host.wan.ip + ':' + (secure ? 443 : 80) + ' weight=3 max_fails=1 fail_timeout=10s' + backup);
							});
						} else {
							hosts.push('server ' + ip + ':' + (secure ? 443 : 80) + ' weight=3 max_fails=1 fail_timeout=10s' + backup);
						}
						external = true;
						counter++;
					}
