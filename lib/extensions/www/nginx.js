...
function generateFile(route, routePath, callback) {
    ...
    NginxConfFile.create(routePath, (err, conf) => {
        ...
        function makeHost(port, secure) {
            let maxFailsDefault = route.source.maxFails || (config.routing.options.nginx && config.routing.options.nginx.maxFails) || 1;
            let failTimeoutDefault = route.source.failTimeout || (config.routing.options.nginx && config.routing.options.nginx.failTimeout) || '15s';
            ...
            for (const item of target) {
                failTimeout = item.source.failTimeout ? item.source.failTimeout : failTimeoutDefault;
                if (item.TargetStatic) {
                    ...
                    if (typeof redirect === 'string') {
                        ...
                        if (url === true) {
                            ...
                            let backup = '';
                            if (item.location && (item.location !== config.parser.location) && route.hasLocalEndpoints) {
                                backup = ' backup';
                            }
                            hosts.add(`server ${uri} weight=3 max_fails=${maxFails} fail_timeout=${failTimeout}${backup}`);
                        }
                    }
                } else {
                    if (!item.location || item.location === config.parser.location) {
                        ...
                        hosts.add(`server ${item.lan3 || item.wan3}:${targetPort} weight=3 max_fails=${maxFails} fail_timeout=${failTimeout}`);
                        counter += 1;
                    } else {
                        const backup = route.hasLocalEndpoints && !secure ? ' backup' : '';
                        maxFails = backup ? 3 : 5;
                        if (route.hasLocalEndpoints && secure && !external) {
                            hosts.add(`server 127.0.0.1:81 weight=3 max_fails=3 fail_timeout=${failTimeout} backup`);
                        }

                        const list = route.hasLocalEndpoints && secure ? hostsExternal : hosts;
                        if (route.hasLocalEndpoints && secure && item.wan3) {
                            list.add(`server ${item.wan3}:443 weight=3 max_fails=${maxFails} fail_timeout=${failTimeout}`);
                        } else {
                            const location = item.location;
                            const wan3 = (location && location.wan3) || item.wan3;
                            if (wan3) {
                                for (const ip of [].concat(wan3)) {
                                    list.add(`server ${ip}:${secure ? 443 : 80} weight=3 max_fails=${maxFails} fail_timeout=${failTimeout}${backup}`);
                                }
                            }
                        }
                        external = true;
