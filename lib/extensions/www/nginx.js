		...
		Object.keys(config._routes).forEach(function (route, index) {
			...
			NginxConfFile.create(routePath, function (err, conf) {
				...
				function makeHost(port, secure) {
					...
					nserver._add('proxy_set_header', 'Secure ' + !!secure);

					var target = config._routes[route];
					if (target.length === 1) {
						var item = target[0];
						if (typeof item === 'string') {
							nserver._add('proxy_pass', item);
							...
							if (config.routing.routes[route].localLocation && !item.external) {
								nserver._add('proxy_pass', 'https://' + (item.lan.ip || item.wan.ip) + ':' + targetPort);
							} else if (!config.routing.routes[route].localLocation && item.external) {
								nserver._add('proxy_pass', 'https://' + (item.wan.ip || config.locations[item.location].ext.web) + ':' + targetPort);
							}
						}
					} else if (!secure) {
						...
						nserver._add('proxy_pass', 'https://' + key);
					}
				}
