...
const { checkExpired } = require('../../acme/utils');
const { mkdirSafe, findFile, checkFileExists } = require('../../utils/fs');
    ...
    for (const [settingKey, settingValue] of settingsSorted) {
        if (settingKey === 'match') {
            for (const [matchKey, matchValue] of Object.entries(settingValue)) {
                ...
                addSettings(matchLocation, matchValue, structuredClone(headers));
            }
        } else if (settingKey.startsWith('if ')) {
            root._add('if', settingKey.replace(/^if /, ''));
            const ifBlock = root.if.at(-1);
            if (isPlainObject(settingValue)) {
                addSettings(ifBlock, settingValue);
            } else {
                for (const item of [settingValue].flat()) {
                    ifBlock._add(item);
                }
            }
        } else if (settingKey.startsWith('location ')) {
            root._add('location', settingKey.replace(/^location /, ''));
            const locationBlock = root.location.at(-1);
            if (isPlainObject(settingValue)) {
                addSettings(locationBlock, settingValue);
            } else {
                for (const item of [settingValue].flat()) {
                    locationBlock._add(item);
                }
            }
            ...
            for (const item of [settingValue].flat()) {
                if (isPlainObject(item) && item?.value !== undefined) {
                    root._add(settingKey, `${item.key ? `${item.key}${' '.repeat(item.keyMaxLength - item.key.length)} ` : ''}${item.value};${item.comment ? `${' '.repeat(item.valueMaxLength - item.value.length)} # ${item.comment}` : ''}`);
                } else {
...
function processACLs(nginx) {
    nginx.__ACL = [];
    if (nginx.deny) {
        const denyList = parseAcls(config.parser, new Set([nginx.deny].flat().filter((x) => x && x !== 'all')));
        nginx.__ACL = nginx.__ACL.concat(denyList.map((x) => {
            if (x.key === undefined) {
                x.key = 'deny';
            }
            return x;
    ...
    if (nginx.allow) {
        const allowList = parseAcls(config.parser, new Set([nginx.allow, '@routers', `@location:${config.parser.location.key}`].flat().filter(Boolean)));
        allowList.push({
            key: 'deny',
            value: 'all',
            comment: 'static entry',
        });
        nginx.__ACL = nginx.__ACL.concat(allowList.map((x) => {
            if (x.key === undefined) {
                x.key = 'allow';
            }
            return x;
    ...
    const keyMaxLength = Math.max.apply(undefined, nginx.__ACL.map((x) => x.key.length));
    const valueMaxLength = Math.max.apply(undefined, nginx.__ACL.map((x) => x.value.length));
    for (let i = 0, till = nginx.__ACL.length; i < till; i++) {
        nginx.__ACL[i].keyMaxLength = keyMaxLength;
        nginx.__ACL[i].valueMaxLength = valueMaxLength;
    }
    return nginx;
}
...
async function generateFile(route, routePath, o) {
    ...
    const listenOptions = (source.listenOptions || config.routing?.options?.nginx?.listenOptions) ?? {};
    ...
    await new Promise((resolve) => {
        NginxConfFile.create(routePath, (err, conf) => {
            ...
            function makeHost({
                ...
                const listenOptionsValue = listenOptions[secure ? 'secure' : 'plain'];
                const listenOptionsString = isDefault && isPlainObject(listenOptions) && typeof listenOptionsValue === 'string' ? ` ${listenOptionsValue}` : '';
                const listen = `${port}${isDefault ? ' default_server' : ''}${secure ? ' ssl' : ''}${listenOptionsString}`;
                ...
                const hasLocalEndpoints = route.hasLocalEndpoints;
                const getLocations = (x) => {
                    const result = [];
                    const type = x.toString();
                    if (type.startsWith('[TargetStatic')) {
                        if (!x.location) {
                            result.push(config.parser.location);
                        } else {
                            result.push([...getLocations(x.location).flat()]);
                        }
                    } else if (type.startsWith('[Server') || type.startsWith('[Router')) {
                        if (!x.location) {
                            result.push(config.parser.location);
                        } else {
                            result.push([...getLocations(x.location).flat()]);
                        ...
                        result.push([...x.endpointsList.filter(y => x !== y).map((y) => getLocations(y)).flat()]);
                ...
                for (const item of route.endpointsList) {
                    ...
                    if (item.TargetStatic) {
                        ...
                        if (typeof redirect === 'string') {
                            ...
                            const hasLocal = hasLocalEndpoints;
                            const backup = hasLocal && item.isRemote && !route.TargetStatic ? ' backup' : '';
                            if (item.isLocal) {
                                flag = true;
                                ...
                                hosts.add(`server ${uri} weight=${upstreamWeight} max_fails=${maxFailsDefault} fail_timeout=${failTimeout}${backup}`);
                                ++counter;
                            } else {
                                remote = true;
                                for (const location of getLocations(item)) {
                                    const backupWan = hasLocal && location !== config.parser.location ? ' backup' : '';
                                    const wan3 = (location && location.wan3) || item.wan3;
                                    if (wan3) {
                                        for (const ip of [wan3].flat()) {
                                            hosts.add(`server ${ip}:${secure ? 443 : 80} weight=${upstreamWeight} max_fails=${maxFailsDefault} fail_timeout=${failTimeout}${backupWan}`);
                                            ++counter;
                                        }
                                    }
                                }
                            }
                        } else if (url !== null && url !== false && (secure || !redirectFromHttp) && !redirect && !hasLocalEndpoints) {
                            remote = true;
                            for (const location of getLocations(item)) {
                                const wan3 = (location && location.wan3) || item.wan3;
                                if (wan3) {
                                    for (const ip of [wan3].flat()) {
                                        hosts.add(`server ${ip}:${secure ? 443 : 80} weight=${upstreamWeight} max_fails=${maxFailsDefault} fail_timeout=${failTimeout}`);
                                        ++counter;
                                    }
                                }
                            }
                        ...
                        const hasLocal = hasLocalEndpoints && item?.routingType?.level6?.[secure ? 'secure' : 'plain'];
                ...
                const proxyPassUrl = `${flag ? staticProtocol : (hasLocalEndpoints || !secure ? 'http' : 'https')}://${key}_${protocol}`;
                let backendLocationKey = '/';

                const headers = {};
                if (!redirectHttps) {
                    if (typeof source.nginx === 'object') {
                        const nginx = remote === true ? {} : {
                            ...source.nginx,
                        };
                        processACLs(nginx);
                    ...
                    if (source.access && !source?.nginx?.allow && !source?.nginx?.deny) {
                        const allowList = parseAcls(config.parser, new Set([source.access, '@routers', `@location:${config.parser.location.key}`].flat().filter(Boolean)));
                    ...
                    if (source.root && !redirectHttps) {
                        ...
                        for (const [staticKey, staticValue] of Object.entries(source.static)) {
                            ...
                            if (typeof staticValue.nginx === 'object') {
                                const nginx = { ...staticValue.nginx };
                                processACLs(nginx);
                                addSettings(staticLocation, nginx, headers);
                            }
                ...
                if (source.upstreamSettings) {
                    upstream._add(source.upstreamSettings);
                }
            ...
            let configString = conf.toString().replaceAll('__ACL ', '');
            fs.writeFile(routePath, configString).then(() => {
                resolve();
            });
            // conf.live(routePath);
            // conf.flush();
        ...
        if (sslOn) {
            ...
            Object.assign(jobs, {
                ...
                'openssl check certificates': async () => {
                    ...
                    const links = await findFile(certPath, /^host-.*\.(crt|key)$/, true, (d) => d.isSymbolicLink());
                    for (const link of links) {
                        certificatesUnlink.push(checkFileExists(link).then((r) => {
                            if (!r) {
                                certificatesUnlink.push(fs.unlink(link).then(() => debug(`certificate cleanup: ${link}: link points to missing file, unlinked`)));
                            }
                        }));
                    }
                    ...
                    for (const item of certificatesChecked) {
                        ...
                        if (expired) {
                            const fileKey = file.replace(/\.crt$/, '.key');
                            certificatesUnlink.push(Promise.all([
                                checkFileExists(file).then((r) => r && fs.unlink(file)),
                                checkFileExists(fileKey).then((r) => r && fs.unlink(fileKey)),
                            ]).then(() => {
                ...
                'openssl generate certificates': ['generate root certificates', 'openssl check certificates', async () => {
                    ...
                    for (const [routeKey, route] of Object.entries(config.parser.targets.map)) {
                        ...
                        if (preset && preset.acmeAny) {
                            ...
                            if (!expirationCheck[accountPath]) {
                                expirationCheck[accountPath] = async () => {
                                    const expiredResult = await checkExpired(exportPath);
                                    expiredResult.forEach((x) => debug(x));
                                };
                            }
                            const sslRoute = route && route.source.ssl;
                            tasks.push(
                    ...
                    try {
                        ...
                        await Promise.all(Object.values(expirationCheck).map((x) => x()));
...
async function generateXFFMapConfig(o) {
    ...
    const map = file`
        map $${o.mainVarName} $${o.mainVarName}_client {
            "~^([^,]*)" $1;
            default $${o.mainVarName};
        }
