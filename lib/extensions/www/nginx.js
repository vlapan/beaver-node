							...
							if (target.length === 1) {
								...
								if (typeof item === 'string') {
									...
									if (config.routing.routes[route].localLocation && !item.external) {
										...
										nlocation._add('proxy_pass', protocol + '://' + config.locations[item.location].ext.web.ip + ':' + (secure ? 443 : 80));
									}
								}
							} else {
								...
								target.forEach(function (item) {
									if (typeof item === 'string') {
										...
										if (config.routing.routes[route].localLocation && !item.external) {
											...
											upstream._add('server ' + (item.wan.ip || config.locations[item.location].ext.web.ip) + ':' + targetPort + ' weight=3 max_fails=1 fail_timeout=2s');
										}
									}
								});

								nlocation._add('proxy_pass', (localLocation ? 'http' : 'https') + '://' + key + '_' + protocol);
							}
						}
