						...
						function makeHost(port, secure) {
							...
							target.forEach(function (item) {
								if (typeof item === 'string') {
									upstream._add('server ' + item.replace(/^https?:\/\//gi, '') + ' weight=3 max_fails=1 fail_timeout=2s');
									...
									if (config.routing.routes[route].localLocation && !item.external) {
										...
										upstream._add('server ' + config.locations[item.location].ext.web.ip + ':' + (secure ? 443 : 80) + ' weight=3 max_fails=1 fail_timeout=2s');
									}
								}
							});
