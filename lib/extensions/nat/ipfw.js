...
const drawTemplate = (o) => file`
    ...
    : | $fw /dev/stdin <<- EOF
        ...
        ${Object.values(o.portForwardAccess.wanToLan).map((x) => `set 2 table service-wan-access-tmp add ${x.srcip},${x.proto},${x.dstport}${x.comment ? `${' '.repeat(o.portForwardWanToLanAccessMaxLength - x.valueLength)} # ${x.comment}` : ''}`).join(' \n        ')}
        ...
        ${Object.values(o.portForwardAccess.lanToWan).map((x) => `set 2 table service-lan-access-tmp add ${x.srcip},${x.proto},${x.srcport},${x.dstip}${x.comment ? `${' '.repeat(o.portForwardLanToWanAccessMaxLength - x.valueLength)} # ${x.comment}` : ''}`).join(' \n        ')}
