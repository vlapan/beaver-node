    ...
    : | $fw /dev/stdin <<- EOF
        ...
        ${
            (o.httpClosed ? [
                o.httpPort.map(
                    (port) => [
                        `add 504 set 2 allow ip from table(${o.httpClosed}) to me dst-port ${port} in // http: allow specific`,
                        `add 504 set 2 deny ip from any to me dst-port ${port} in // http in: deny others`,
                    ].join(' \n        ')
                ).join(' \n        '),
            ] : [
        ...
        ${
            (o.sshClosed ? [
                o.sshPort.map(
                    (port) => [
                        `add 504 set 2 allow tcp from table(${o.sshClosed}) to me dst-port ${port} in // ssh: allow specific`,
                        `add 504 set 2 deny tcp from any to me dst-port ${port} in // ssh: deny others`,
                    ].join(' \n        ')
                ).join(' \n        '),
            ] : [
                o.sshPort.map((port) => `add 504 set 2 allow tcp from any to me dst-port ${port} in // ssh: allow all`).join(' \n        '),
            ]).join('\n        ')
        }
        ...
        ${o.httpPort.map((port) => `add 506 set 2 allow ip from me to not me src-port ${port} out // http out`).join(' \n        ')}
        ${o.sshPort.map((port) => `add 506 set 2 allow tcp from me to not me src-port ${port} out // ssh out`).join(' \n        ')}
        ...
        add 506 set 2 allow ip from me to not me src-port ${o.tincPort} out // tinc out
        add 506 set 2 allow ip from me to not me src-port ${o.beaverPort} out // beaver-api out
