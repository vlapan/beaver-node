...
const drawTemplate = (o) => file`
    ...
    : | $fw /dev/stdin <<- EOF
        ...
        ${o.httpPort.map((port) => `add 506 set 2 allow ip from me src-port ${port} to not me out // http out`).join(' \n        ')}
        ${o.sshPort.map((port) => `add 506 set 2 allow tcp from me src-port ${port} to not me out // ssh out`).join(' \n        ')}
        ...
        add 506 set 2 allow ip from me src-port ${o.tincPort} to not me out // tinc out
        add 506 set 2 allow ip from me src-port ${o.beaverPort} to not me out // beaver-api out
