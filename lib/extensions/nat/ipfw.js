...
const drawTemplate = (o) => file`
    ...
    : | $fw /dev/stdin <<- EOF
        nat 1 config ${o.iface ? `if ${o.iface}` : `ip ${o.externalIp}`} unreg_only \\
        ...
        ${o.tincRemote.map((item) => `set 2 table tinc-tap-l6-hosts-remote add ${item.ip} ${item.tapIp} \t #   ${item.comment}`).join('\n        ')}


        set 2 table tinc-tap-l6-hosts-local create or-flush
        ${o.tincLocal.map((item) => `set 2 table tinc-tap-l6-hosts-local add ${item.ip} \t\t\t #   ${item.comment}`).join('\n        ')}

        ${
            Object.entries(o.tables).map((x) => {
                const table = [];
                table.push(`set 2 table ${x[0]} create or-flush`);
                const max = Math.max.apply(undefined, x[1].map((x) => x.value.length));
                for (const item of x[1]) {
                    if (!item.value) {
                        continue;
                    }
                    table.push(`set 2 table ${x[0]} add ${item.key ? `${item.key} ` : ''}${item.value}${item.comment ? `${' '.repeat(max - item.value.length)} # ${item.comment}` : ''}`);
                }
                return table.join('\n        ');
            }).join('\n\n        ')
        }
