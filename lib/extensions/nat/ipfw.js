...
module.exports = {
    generate(callback) {
        ...
        logger.log('info', '======================================================================');
        logger.log('info', 'IPFW configuration generation');
        logger.log('info', '----------------------------------------------------------------------');
        ...
        config.parser.routers.list.filter((router) => {
            return undefined !== router.tcpShift;
        });
        ...
        Object.keys(config.servers).filter((vmKey) => {
            ...
            const beaverRedirect = `redirect_port tcp ${ip}:${1001} ${beaverPortSrc}`;
            logger.log('debug', 'append rule:', beaverRedirect);
            ipfw += ` \\\n    ` + beaverRedirect;
        ...
        Object.keys(config.servers).filter((vmKey) => {
            ...
            Object.keys(ports).forEach((portSrc) => {
                ...
                if (!redirectType || redirectType === 'tcp') {
                    const redirect = `redirect_port tcp ${ip}:${portTgt} ${portSrc}`;
                    logger.log('debug', 'append rule:', redirect);
                    ipfw += ` \\\n    ` + redirect;
                }
                if (!redirectType || redirectType === 'udp') {
                    const redirect = `redirect_port udp ${ip}:${portTgt} ${portSrc}`;
                    logger.log('debug', 'append rule:', redirect);
                    ipfw += ` \\\n    ` + redirect;
                }
            });
