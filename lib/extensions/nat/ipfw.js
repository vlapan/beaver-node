	...
	generate: function (callback) {
		...
		Object.keys(config.vms).filter(function (vmKey) {
			...
			Object.keys(ports).forEach(function (portSrc) {
				...
				var redirect = ' \\\n    redirect_port tcp ' + vm.lan.ip + ':' + portTgt + ' ' + portSrc;
				redirect += ' \\\n    redirect_port udp ' + vm.lan.ip + ':' + portTgt + ' ' + portSrc;
		...
		output += '${fw} add 501 skipto 900 ip from 192.168.0.0/16 to 192.168.0.0/16\n';
		...
		output += '${fw} add 602 nat 1 ip from 192.168.0.0/16 to not 192.168.0.0/16\n';
