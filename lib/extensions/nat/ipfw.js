    ...
    : | $fw /dev/stdin <<- EOF
        ...
        ${
            (o.sshClosed ? [
                o.sshPort.map(
                    (port, i) => [
                        `add ${11020 + i} set 2 allow tcp from table(${o.sshClosed}) to me  dst-port ${port} in // ssh: allow specific`,
                        `add ${11020 + i} set 2 deny  tcp from any to me  dst-port ${port} in // ssh: deny others`,
                        `add ${31020 + i} set 2 allow tcp from table(${o.sshClosed}) to me6 dst-port ${port} in // ssh: allow specific`,
                        `add ${31020 + i} set 2 deny  tcp from any to me6 dst-port ${port} in // ssh: deny others`,
                    ].join(' \n        ')
                ).join(' \n        '),
            ] : [
                o.sshPort.map((port, i) => `add ${11020 + i} set 2 allow tcp from any to me  dst-port ${port} in // ssh: allow all`).join(' \n        '),
                o.sshPort.map((port, i) => `add ${31020 + i} set 2 allow tcp from any to me6 dst-port ${port} in // ssh: allow all`).join(' \n        '),
            ]).join('\n        ')
        }
