...
module.exports = {
    generate(callback) {
        ...
        output += '${fw} add 504 allow tcp from any to me dst-port 22,27 in\n';
        output += '${fw} add 504 allow tcp from any to me dst-port 80,443 in\n';
        output += '${fw} add 504 allow tcp from any to me dst-port 53,655 in\n';
        output += '${fw} add 504 allow udp from any to me dst-port 53,655 in\n';

        output += '${fw} delete 506 || true\n';
        output += '${fw} add 506 allow ip from any to any via lo0\n';

        output += '${fw} delete 508 || true\n';
        output += '${fw} add 508 count ip4 from me to not me out\n';
        output += '${fw} add 508 count ip6 from me to not me out\n';
        output += '${fw} add 508 count tcp from me to not me out\n';
        output += '${fw} add 508 count udp from me to not me out\n';
        output += '${fw} add 508 allow ip from me to not me out\n';

        output += '${fw} delete 518 || true\n';
        output += '${fw} add 518 nat 1 ip from any to ' + externalIp + ' in\n';
        output += '${fw} add 518 skipto 700 ip from 10.0.0.0/8 to 10.0.0.0/8\n';
        output += '${fw} add 518 skipto 700 ip from 172.16.0.0/12 to 172.16.0.0/12\n';
        output += '${fw} add 518 skipto 700 ip from 192.168.0.0/16 to 192.168.0.0/16\n';

        output += '${fw} delete 608 || true\n';

        const { iface } = config.parser.server.source.wan;
        output += `\${fw} add 608 nat 1 ip from 10.0.0.0/8 to any out${iface ? ' xmit ' + iface : ''}\n`;
        output += `\${fw} add 608 nat 1 ip from 172.16.0.0/12 to any out${iface ? ' xmit ' + iface : ''}\n`;
        output += `\${fw} add 608 nat 1 ip from 192.168.0.0/16 to any out${iface ? ' xmit ' + iface : ''}\n`;
        ...
        output += '${fw} add 900 count ip4 from any to any\n';
        output += '${fw} add 900 count ip6 from any to any\n';
        output += '${fw} add 900 count ip from not me to me in\n';
