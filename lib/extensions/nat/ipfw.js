    ...
    : | $fw /dev/stdin <<- EOF
        ...
        ${
            o.portForwardAccess.privateMaxLength === 0 ? '' : [
                ...
                Object.values(o.portForwardAccess.private).map((x) => `set 2 table service-port-forward-private-tmp add ${x.proto},${o.externalIp},${x.dstport} ${1000 + x.id}${x.comment ? `${' '.repeat(o.portForwardAccess.privateMaxLength - x.valueLength)} # ${x.comment}` : ''}`).join('\n        '),
                ...
                Object.entries(o.portForwardAccess.servers).map(([serverKey, server]) => {
                    return `add 511 set 2 drop ip from not table(${server.sourceTableKey}) to any in tagged ${1000 + server.id}`;
                }).join('\n        '),
                '',
            ].join('\n        ')
        }
...
function getObject(parser, argv) {
    ...
    const portForwardAccess = parser.location.servers.list.filter((x) => x.source.access && x.tcpShift && x.routingType?.level3 && myZone === (x.location.source.zone ?? '')).reduce((o, x, i) => {
        ...
        if (result.count > 0) {
            ...
            result.sourceTableKey = `service-port-forward-source-${tableHash}`;
            if (typeof o.acls[tableHash] === 'undefined') {
                o.acls[tableHash] = 1;
