...
const { checkMigrate, getZone, checkExpired } = require('../../acme/utils');
    ...
    async generate(o) {
        ...
        async function processRoute(route) {
            ...
            if (preset && preset.acmeAny) {
                ...
                if (!presetCleanup[preset.key]) {
                    presetCleanup[preset.key] = (async () => {
                        try {
                            const expiredResult = await checkExpired(workdirPath);
                ...
                await presetCleanup[preset.key];
