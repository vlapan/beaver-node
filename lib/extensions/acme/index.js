...
module.exports = {
    async generate(o) {
        ...
        for (const [presetKey, presetSrc] of Object.entries(presets)) {
            const preset = openssl.preparePreset(presetSrc);
        ...
        for (const route of config.parser.targetListWeb) {
            ...
            if (preset && (preset.type === 'acme' || preset.type === 'acme-http-01' || preset.type === 'acme-dns-01') && accounts[preset.key]) {
                ...
                if (!await checkFileExists(acmeAccountKeyPath)) {
                    debug(`${preset.key}: acme account key: not found`);
