            ...
            if (preset && (preset.type === 'acme' || preset.type === 'acme-http-01' || preset.type === 'acme-dns-01')) {
                ...
                if (!accounts[preset.key]) {
                    ...
                    if (preset.master) {
                        ...
                        for (const key of Object.keys(config.parser.routers.map)) {
                            if (key !== preset.master) {
                                account.slaves.push(key);
                            }
                        }
                    } else {
                ...
                if (preset.type === 'acme-dns-01') {
                    ...
                    if (preset.master === config._hostname) {
                        ...
                        const ns = [`@ IN NS ${preset.master}.`];
                        // const ns = config.parser.routers.list.reduce((a, b) => {
                        //     a.push(`@ IN NS ${b.source.key}.`);
                        //     return a;
                        // }, []);
