...
module.exports = {
    generate(callback) {
        ...
        const ns = [];
        const routers = [];
        const routersByLocation = config._routers.reduce((a, b) => {
            a[b.location] = b;
            return a;
        }, {});
        Object.keys(routersByLocation).forEach((locationKey) => {
            const location = config.locations[locationKey];

            if (location.routing.external) {
                ns.push({
                    host: (location.ext && location.ext.tcp && location.ext.tcp.ip || location.name) + '.',
                });
            } else {
                routersByLocation[locationKey].forEach((router) => {
