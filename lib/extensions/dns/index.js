...
module.exports = {
    async generate(o) {
        ...
        function processAppendKey(k, a) {
            const server = o.config.parser.servers.map[k];
            if (server) {
                const wan3 = server.wan3;
                if (wan3) {
                    a.add(`${wan3}/32; # ${server.location.toString()}: ${server.toString()}: WAN3`);
                }
                const wan36 = server.wan36;
                if (wan36) {
                    a.add(`${wan36}/128; # ${server.location.toString()}: ${server.toString()}: WAN36`);
                }
                const lan3 = server.lan3;
                if (lan3) {
                    a.add(`${lan3}/32; # ${server.location.toString()}: ${server.toString()}: LAN3`);
                }
                return a;
            }
            const ipVersion = net.isIP(k);
            const netmask = ipVersion === 4 ? '/32' : (ipVersion === 6 ? '/128' : '');
        ...
        function printZoneMasterConf(type, domain) {
            ...
            if (alsoNotify && type === 'global') {
                ...
                for (const item of [].concat(alsoNotify)) {
                    processAppendKey(item, alsoNotifySet);
                }
                result += '    also-notify {\n';
        ...
        function printZoneSlaveConf(type, domain) {
            ...
            if (domain.masters) {
                for (const item of [].concat(domain.masters)) {
                    processAppendKey(item, mastersSet);
                }
            }
