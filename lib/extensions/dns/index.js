...
module.exports = {
    async generate(o) {
        ...
        function splitLong(txt) {
            if (txt.length < 255) {
                return ~txt.indexOf(' ') ? `"${txt}"` : txt;
            }
            return `(\"${txt.match(/(.{1,255})/g).join('"\n"')}\")`;
        }
            ...
            for (const item of obj.list) {
                ...
                (Array.isArray(value) ? value : [value]).forEach((item) => {
                    ...
                    if (typeof keys.override === 'function') {
                        ...
                        entry[keys.value || 'value'] = (keys.value === 'txt' ? splitLong(item) : item) + comment;
                    }
                    entries.push(entry);
                });
            }
        ...
        function processAppendKey(k, a, n = true, z = false) {
            if (k.startsWith('acl:')) {
                const key = k.replace(/^acl:/, '');
                const acl = o.config?.routing?.options?.acls?.[key] ?? o.config?.routing?.options?.dns?.acl?.[key];
                if (Array.isArray(acl)) {
                    if (n === false || z === true) {
                        for (const item of acl) {
                            processAppendKey(item, a, n, z);
                        }
                    } else {
                        a.set(`"${key}"`, 'ACL');
                    }
        ...
        function printZoneMasterConf(type, domain) {
            ...
            if (options.zonesStats === true || domain.source['stats'] === true) {
                result += `    zone-statistics full;\n`;
            }
            result += '};';
        ...
        const aclList = o.config?.routing?.options?.acls ?? o.config?.routing?.options?.dns?.acl;
        const aclUsed = [];
        ...
        // LOCAL VIEWS
        {
            ...
            for (const lan of lans) {
                ...
                viewResult += '    notify explicit;\n';
                viewResult += '    recursion yes;\n';
                viewResult += `    allow-recursion { ${lan.networkCidr}; };\n`;
                viewResult += `    allow-query { ${lan.networkCidr}; };\n`;
                viewResult += `    allow-query-cache { ${lan.networkCidr}; };\n`;
        ...
        // GLOBAL VIEW
        {
            ...
            const allowRecursion = new Map();
            allowRecursion.set('127.0.0.1/32', 'localhost IPV4');
            allowRecursion.set('::1/128', 'localhost IPV6');

            const net3 = o.config.parser.location.net3;
            if (net3) {
                if (net3.list) {
                    for (const item of net3.list) {
                        allowRecursion.set(`${item.networkCidr}`, `${locationKey}: NET3`);
                    }
                } else if (net3.networkCidr) {
                    allowRecursion.set(`${net3.networkCidr}`, `${locationKey}: NET3`);
                }
            }

            const wan36 = o.config.parser.location.wan36;
            if (wan36) {
                const wan36List = [wan36].flat();
                for (const item of wan36List) {
                    const netmask = item.match(/\/\d+$/);
                    allowRecursion.set(`${item}${netmask ? '' : '/128'}`, `${locationKey}: WAN36`);
                }
            }
            ...
            for (const server of o.config.parser.servers.list) {
                if (server.location === o.config.parser.server.location) {
                    ...
                    if (wan3) {
                        allowRecursion.set(`${wan3}/32`, `${server.location.toString()}: ${server.toString()}: WAN3`);
                    }
                    const wan36 = server.wan36;
                    if (wan36) {
                        allowRecursion.set(`${wan36}/128`, `${server.location.toString()}: ${server.toString()}: WAN36`);
                    }
                }
            }
            ...
            viewResult += `        ${alignComments(allowRecursion).join('\n        ')}\n`;
            viewResult += '    };\n';
            viewResult += '    allow-query { any; };\n';
            viewResult += '    include "default.zones";\n';
            viewResult += `    include "${prefixMaster}/acme.conf";\n`;
            viewResult += '\n';
            ...
            for (const domain of domains.list) {
                for (const key of [domain.source['allow-transfer'], domain.source['also-notify'], domain.source['slaves']].flat().filter(Boolean)) {
                    if (key.startsWith('acl:')) {
                        const aclName = key.replace(/^acl:/, '');
                        if (aclList?.[aclName]) {
                            aclUsed.push(aclName);
                        }
                    }
                }
                domain.keyClean = domain.key.replace(/^\./, '');
        ...
        if (typeof aclList === 'object') {
            ...
            for (const [aclName, acl] of Object.entries(aclList)) {
                if (!Array.isArray(acl) || !aclUsed.includes(aclName)) {
                    continue;
                }
                const a = new Map();
                for (const item of acl) {
                    processAppendKey(item, a, true, true);
                }
