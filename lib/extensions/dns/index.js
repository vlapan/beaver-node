    ...
    async generate(o) {
        ...
        // GLOBAL VIEW
        {
            ...
            if (matchClients.length) {
                viewResult += `view ${viewKey}-trust {\n`;
                viewResult += `    match-clients { ${matchClients.join(';')}; };\n`;
                viewResult += '    recursion yes;\n';
                viewResult += '    include "/usr/local/etc/namedb/default.zones";\n';
                viewResult += '    include "/usr/local/etc/namedb/master/beaver/acme.conf";\n';
                viewResult += '    include "/usr/local/etc/namedb/master/beaver/global.conf";\n';
                viewResult += '};\n';
            }
            ...
            for (const domain of domains) {
                if (domain.mode === 'static') {
                    ...
                    for (const route of config.parser.targetListWeb) {
                        ...
                        if (preset && preset.type === 'acme-dns-01') {
                    ...
                    globalZones.push(`${zoneConf.split('\n').join('\n')}`);
            ...
            await fs.writeFile(`${zonesPath}/global.conf`, globalZones.join('\n'));
        }
