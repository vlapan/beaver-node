...
module.exports = {
	generate: function (callback) {
		...
		var selfRoute = routersFiltered.filter(function(router) {
			return router.wan.ip === config._hostConfig.wan.ip;
		});
		if (selfRoute.length) {
			routersFiltered = [config._hostConfig];
		} else {
			...
			for (var i = 0, till = item.length; i < till; i++) {
				...
				if (typeof target === 'object') {
					...
					if (target.wan) {
						...
						var routeRouters = routersFiltered.slice(0);
						if (!sameLocation) {
							routeRouters = routers.filter(function(router) {
