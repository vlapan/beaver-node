...
module.exports = {
    async generate(o) {
                ...
                switch (cmd) {
                    case 'locations': {
                        for (const location of o.config.parser.locations.list) {
                            processAppendKey(location.key, a, n);
                        }
                        break;
                    }
                    case 'location': {
                        processAppendKey(key, a, n);
                        break;
                    }
                    case 'zone': {
                        for (const location of o.config.parser.locations.list) {
                    ...
                    case 'routers': {
                        for (const router of o.config.parser.routers.list) {
                            processAppendKey(router.key, a, n);
                        }
                        break;
                    }
                    case 'servers': {
                        for (const server of o.config.parser.servers.list) {
                            processAppendKey(server.key, a, n);
                        }
                        break;
                    }
                    case 'server': {
            ...
            for (const lan of lans) {
                ...
                for (const domain of viewSource.list) {
                    ...
                    if (domain.mode === 'static') {
                        domain.zonePath = `${o.argv.dnsPath}/${viewKey}/${domain.keyClean}.dns`;
                        ...
                        domain.zonePath = `${o.argv.dnsSlavePath}/beaver-${viewKey}-${domain.keyClean}.dns`;
        ...
        // GLOBAL VIEW
        {
            ...
            viewResult += '    };\n';
            viewResult += '    include "/usr/local/etc/namedb/default.zones";\n';
            viewResult += `    include "/usr/local/etc/namedb/${o.argv.dnsPath}/acme.conf";\n`;
            viewResult += '\n';
