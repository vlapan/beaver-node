...
module.exports = {
    async generate(o) {
        ...
        function processAppendKey(k, a, n = true) {
            const location = o.config.parser.locations.map[k];
            if (location) {
                const net3 = location.net3;
                if (net3) {
                    if (net3.list) {
                        for (const item of net3.list) {
                            a.add(`${n === false ? item.ip : item.networkCidr};\t\t\t# ${location.toString()}: NET3`);
                        }
                    } else if (net3.networkCidr) {
                        a.add(`${n === false ? net3.ip : net3.networkCidr};\t\t\t# ${location.toString()}: NET3`);
                    }
                }
                ...
                if (wan36) {
                    for (const item of [wan36].flat()) {
                        ...
                        a.add(`${n === false ? item.replace(/\/\d+$/, '') : (netmask ? item : `${item}/128`)};\t# ${location.toString()}: WAN36`);
                    }
                }
                ...
                if (lans) {
                    if (lans.list) {
                        for (const item of lans.list) {
                            a.add(`${n === false ? item.ip : item.networkCidr};\t\t\t# ${location.toString()}: LANS`);
                        }
                    } else if (lans.networkCidr) {
                        a.add(`${n === false ? lans.ip : lans.networkCidr};\t\t\t# ${location.toString()}: LANS`);
                    }
                }
                return a;
            }
            ...
            if (type === 'global') {
                ...
                if (slaves.length > 0) {
                    ...
                    for (const location of o.config.parser.locations.list) {
                        processAppendKey(location.key, allowTransferSet);
                    }
                    for (const server of o.config.parser.routers.list) {
                        processAppendKey(server.key, allowTransferSet);
                    }
                }
            }
            const allowTransfer = Array.from(allowTransferSet).join('\n        ');
