                ...
                if (domain.mode === 'static') {
                    ...
                    for (const route of o.config.parser.targetListWeb) {
                        ...
                        if (preset && preset.acmeDns) {
                            if (route.key.match(`${domain.keyClean}$`) || (typeof preset.mirrorDomainSuffix === 'string' && preset.mirrorDomainSuffix.endsWith(domain.keyClean))) {
                                const key = `_acme-challenge${(`.${route.key}`).replace(new RegExp(`.${domain.keyClean}$`), '')}`;
