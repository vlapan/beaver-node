        ...
        // GLOBAL VIEW
        {
            ...
            if (net3) {
                if (net3.list) {
                    for (const item of net3.list) {
                        allowRecursion.add(`${item.networkCidr}; # ${locationKey}: NET3: ${item.toString()}`);
                    }
                } else if (net3.networkCidr) {
                    allowRecursion.add(`${net3.networkCidr}; # ${locationKey}: NET3: ${net3.toString()}`);
                }
            }

            const wan36 = o.config.parser.location.wan36;
            if (wan36) {
                if (Array.isArray(wan36)) {
                    for (const item of wan36) {
                        const netmask = item.match(/\/[0-9]+$/);
                        allowRecursion.add(`${item}${netmask ? '' : '/128'}; # ${locationKey}: WAN36`);
                    }
                } else {
                    const netmask = wan36.match(/\/[0-9]+$/);
                    allowRecursion.add(`${wan36}${netmask ? '' : '/128'}; # ${locationKey}: WAN36`);
                }
            }
            ...
            for (const server of o.config.parser.servers.list) {
                if (server.location === o.config.parser.server.location) {
                    ...
                    if (wanIp) {
                        allowRecursion.add(`${wanIp}/32; # ${server.location.toString()}: ${server.toString()}`);
                    }
                    const wanIp6 = server?.source?.wan?.ipv6;
                    if (wanIp6) {
                        allowRecursion.add(`${wanIp6}/128; # ${server.location.toString()}: ${server.toString()}`);
                    }
                }
            }
