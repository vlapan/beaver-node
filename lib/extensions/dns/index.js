...
module.exports = {
    async generate(o) {
        ...
        function processAppendKey(k, a, n = true) {
            const kk = typeof k === 'object' ? k.value : k;

            if (kk.startsWith('acl:')) {
                const key = kk.replace(/^acl:/, '');
                const acl = o.config?.routing?.options?.acls?.[key];
        ...
        const aclList = o.config?.routing?.options?.acls;
        const aclUsed = [];
        ...
        // GLOBAL VIEW
        {
            ...
            for (const domain of domains.list) {
                for (const item of [domain.source['allow-transfer'], domain.source['also-notify']].flat().filter(Boolean)) {
