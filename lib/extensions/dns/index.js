...
module.exports = {
    generate(callback) {
        ...
        const domainsLocal = config.parser.buildDnsView(config.parser.location.lans).toSourceObject();
        for (const [domainKey, domainLocal] of Object.entries(domainsLocal)) {
            domainLocal.key = domainKey.replace(/^./, '');
        }
        ...
        for (const domain of domains) {
            const domainLocal = domainsLocal[domain.keySource];
            if (domain.mode === 'static') {
                ...
                generateDomainFile(domainLocal, path.normalize(`${zonesLocalPath}/${domain.key}.dns`));
                zonesConf.local.push(printZoneMasterConf('local', domainLocal));
            } else if (domain.mode === 'dedicated') {
                generateDomainFile(domain, path.normalize(`${zonesGlobalPath}/${domain.key}.dns`));
                zonesConf.global.push(printZoneMasterConf('global', domain));
            } else if (domain.mode === 'slave') {
                zonesConf.global_slave.push(printZoneSlaveConf('global', domain));
                zonesConf.local_slave.push(printZoneSlaveConf('local', domainLocal));
            }
        }
