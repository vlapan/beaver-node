...
const M = {
    ...
    async generate(o) {
        ...
        try {
            ...
            const hookProcess = async (hook) => {
                debug(`executing hook: ${hook} ${o.config._outputPath}`);
                try {
                    await M.spawn({
                        command: hook,
            ...
            if (await checkDirectoryExists(hookDirPath)) {
                ...
                for (const hookPath of hooksPath.sort()) {
                    await hookProcess(hookPath);
                }
            }
            if (!o.argv.hook || !(await modeRX(o.argv.hook))) {
                const hookDefaultPath = path.resolve(`${o.argv.home}/hook`);
                if (await modeRX(hookDefaultPath)) {
                    o.argv.hook = hookDefaultPath;
                }
            }
            if (o.argv.hook) {
                await hookProcess(o.argv.hook);
            }
