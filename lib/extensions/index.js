...
module.exports = {
    ...
    generate(callback) {
        ...
        function cb(err, result) {
            const { slack } = config.parser.monitoring && config.parser.monitoring.notify && config.parser.monitoring.notify.source || {};
            if (typeof slack === 'object' && typeof slack.hook === 'string' && ~slack.hook.indexOf('https://hooks.slack.com/')) {
                const message = `${config._hostname}: ${err ? `Configuration failed: "${err}"` : 'Configuration done!'}`;
