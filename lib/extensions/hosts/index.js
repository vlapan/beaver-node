	...
	generate: function (callback) {
		...
		function add(type, key) {
			...
			for (var i = 0, till = item.length; i < till; i++) {
				...
				if (localRoute) {
					if (item.length === 1) {
						targets = [target.lan.ip];
					}
				} else {
					if (item.length === 1 && target.wan && target.wan.ip) {
						targets = [target.wan.ip];
					}
				}
			}
		...
		var outputFile = path.normalize(config._outputPath + '/hosts');
		...
		function generateFile(entries) {
			...
			entries.forEach(function (entry) {
				var s = ' ';
				if (entry.ip.length < max) {
					s = s + s.repeat(max - entry.ip.length);
				}
				if (!entry.domainFound) {
					return;
				}
				domains.forEach(function (domain) {
					output += entry.ip + s + entry.name + '.' + domain.key + '\n';
				});
			});
