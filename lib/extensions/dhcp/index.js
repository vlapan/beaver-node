...
module.exports = {
    async generate(o) {
        ...
        const options = o.config.routing.options && typeof o.config.routing.options.dhcp === 'object' ? o.config.routing.options.dhcp : {};
        ...
        for (const host of view.list) {
            ...
            if (domainName) {
                if (options.useDomainName) {
                    output += ` option domain-name "${domainName}";`;
                } else if (options.useDomainSearch) {
                    output += ` option domain-search "${domainName}";`;
                } else {
                    output += ` option domain-search "invalid";`;
                }
            } else {
                output += ` option domain-search "invalid";`;
            }
            ...
            const resolvers = [];
            if (host.resolver) {
                resolvers.push(host.resolver);
            }
            if (options.resolvers) {
                resolvers.push(options.resolvers);
            }
            if (resolvers.length > 0) {
                const nameServers = resolvers.flat().filter(Boolean);
                if (nameServers.length > 0) {
                    output += ` option domain-name-servers ${nameServers.join(', ')};`;
                }
            }
