...
module.exports = {
	generate: function (callback) {
		...
		var gateway = routing && routing.gateway ? (Array.isArray(routing.gateway) ? routing.gateway.join(',') : routing.gateway) : false;
		...
		for (var i = 0, till = keys.length; i < till; i++) {
			...
			output += 'host ' + key + ' {';
			output += ' option host-name "' + item.name + '";';
			if (item.wan !== true && gateway) {
