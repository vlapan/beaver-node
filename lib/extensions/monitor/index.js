...
function nodeAccessDetail(structure, key, node) {
    ...
    if (vm.wan && vm.wan.ip) {
        if (os && os.ssh) {
            ...
            tcp.push({
                ...
                ip: vm.wan.ip,
                port: port,
                subject: `tcp-conn-ssh: ${key}, ssh-wan, ip=${vm.wan.ip}, port=${port}`,
                notify: vm.notify,
            });
        ...
        if (os && os.rdp) {
            ...
            tcp.push({
                ...
                ip: vm.wan.ip,
                port: port,
                subject: `tcp-conn-rdp: ${key}, rdp-wan, ip=' + ${vm.wan.ip}, port=${port}`,
                notify: vm.notify,
            });
    ...
    if (vm.lan && vm.lan.ip) {
        ...
        tcp.push({
            ...
            ip: vm.lan.ip,
            port: port,
            subject: `tcp-conn-ssh: ${key}, ssh-lan, ip=${vm.lan.ip}, port=${port}`,
            notify: vm.notify,
        });
        ssh.push(`ssh ${vm.lan.ip}`);
    }
    if (wan3 && level3 && level3['22/tcp'] && server.tcpShift) {
        ...
        tcp.push({
            ...
            ip: wan3,
            port: port,
            subject: `tcp-conn-ssh: ${key}, ssh-fwd, ip=${wan3}, port=${port}`,
            notify: vm.notify,
        });
        ssh.push(`ssh ${wan3} -p ${port}`);
    }
    if (wan3 && level3 && level3['389/tcp'] && server.tcpShift) {
        ...
        tcp.push({
            ...
            ip: wan3,
            port: port,
            subject: `tcp-conn-rdp: ${key}, rdp-fwd, ip=${wan3}, port=${port}`,
            notify: vm.notify,
        });
