...
function generateTcpTests() {
    ...
    for (const [key, server] of Object.entries(config.parser.servers.map)) {
        ...
        const level3Rdp = Object.entries(level3).find(([k]) => k.startsWith('389/tcp') || k === '389');
        ...
        const sshPort = level3Ssh && Number.parseInt(level3Ssh[1], 10);
        const rdpPort = level3Rdp && Number.parseInt(level3Rdp[1], 10);
