...
module.exports = {
    generate: function (callback) {
        ...
        const monitorConfigPath = `${config._outputPath}/monitor.json`;

        logger.log('info', `config path: ${monitorConfigPath}`);
        logger.log('info', `monitoring ${monitorConfig.disabled ? 'disabled' : 'enabled'}`);

        fs.writeFile(monitorConfigPath, JSON.stringify(monitorConfig, null, 4), function (err) {
