    ...
    Object.keys(config.parser.targets.source).forEach(function (key) {
        ...
        if (url) {
            ...
            addresses.forEach(function (address) {
                ...
                let httpPort = localLocation && config.routing.types[address.type || 'unix'].level6.plain | 0;
                if (httpPort === 80) {
                    httpPort = undefined;
                }
                web.push({
                ...
                let httpsPort = localLocation && config.routing.types[address.type || 'unix'].level6.secure | 0;
                if (httpsPort === 443) {
                    httpsPort = undefined;
                }
                if (httpsPort !== 80) {
