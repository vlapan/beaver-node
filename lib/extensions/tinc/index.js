...
const tinc = {
    async generate(o) {
        ...
        for (const [key, server] of Object.entries(o.config.servers)) {
            ...
            const serverZone = (o.config.parser.servers.map[key].location.source.zone ?? '');
        ...
        for (const [net, hosts] of Object.entries(nets)) {
            ...
            for (const v of o.config.parser.servers.list) {
                if (v.source.net && v.source?.lan?.ip && v.location !== o.config.parser.server.location && myZone === (v.location.source.zone ?? '')) {
    ...
    generateTincUpHook(netPath, item) {
        let output = '';
        output += '#!/bin/sh\n';
        output += `logger -t "tinc.${item.name}[$PPID]" "tinc-up, name=$NAME, iface=$INTERFACE"\n`;
    ...
    generateTincDownHook(netPath, item) {
        let output = '';
        output += '#!/bin/sh\n';
        output += `logger -t "tinc.${item.name}[$PPID]" "tinc-down, name=$NAME, iface=$INTERFACE"\n`;
        if (Array.isArray(item.vip) && item.vip.length > 0) {
            for (let i = 0, till = item.vip.length; i < till; i++) {
                const vip = item.vip[i];
                if (typeof vip === 'object' && typeof vip.ip === 'string') {
    ...
    generateHostUpHook(netPath, item) {
        let output = '';
        output += '#!/bin/sh\n';
        output += `logger -t "tinc.${item.name}[$PPID]" "host-up, iface=$INTERFACE, node=$NODE, remoteaddress=$REMOTEADDRESS, remoteport=$REMOTEPORT"\n`;
        return fs.writeFile(path.normalize(`${netPath}/host-up`), output);
    },
    generateHostDownHook(netPath, item) {
        let output = '';
        output += '#!/bin/sh\n';
        output += `logger -t "tinc.${item.name}[$PPID]" "host-down, iface=$INTERFACE, node=$NODE, remoteaddress=$REMOTEADDRESS, remoteport=$REMOTEPORT"\n`;
    ...
    generateSubnetUpHook(netPath, item, mode = 'switch') {
        let output = '';
        output += '#!/bin/sh\n';
        output += `logger -t "tinc.${item.name}[$PPID]" "subnet-up, iface=$INTERFACE, node=$NODE, remoteaddress=$REMOTEADDRESS, remoteport=$REMOTEPORT, subnet=$SUBNET, weight=$WEIGHT"\n`;
        if (mode === 'router') {
            output += 'DIR=$(dirname -- "$(readlink -f -- "$0";)")\n';
    ...
    generateSubnetDownHook(netPath, item, mode = 'switch') {
        let output = '';
        output += '#!/bin/sh\n';
        output += `logger -t "tinc.${item.name}[$PPID]" "subnet-down, iface=$INTERFACE, node=$NODE, remoteaddress=$REMOTEADDRESS, remoteport=$REMOTEPORT, subnet=$SUBNET, weight=$WEIGHT"\n`;
        if (mode === 'router') {
            output += 'DIR=$(dirname -- "$(readlink -f -- "$0";)")\n';
            output += 'if [ "$REMOTEADDRESS" ]; then\n';
            output += '    IP=${SUBNET%/*}\n';
