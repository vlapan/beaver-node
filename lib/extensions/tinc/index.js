...
const tinc = {
    async generate(o) {
        ...
        for (const [net, hosts] of Object.entries(nets)) {
            ...
            for (const [key, item] of Object.entries(hosts)) {
                ...
                const servers = config.parser.servers.list.reduce((o, v) => {
                    if (v.source.net && v.source.lan && v.source.lan.ip) {
                        const group = v.location.key === item.location ? 'local' : 'remote';
                        o[group].push(v);
                    }
                    return o;
                }, {
                ...
                console.log(servers);
                servers.local.forEach((item) => {
                    hostFile += `# ${item.key}\n`;
                    hostFile += `Subnet = ${item.source.lan.ip}/32\n`;
                });
                ...
                promises.push(fs.writeFile(path.normalize(`${hostsPath}/${key.replace(/[-.]/gi, '_')}`), hostFile).then(() => debug(`${net}: host=${key}, ip=${item.tap.ip}: done`)));
            }
            ...
            promises.push(Promise.all([
                ...
                tinc.generateTincConfHook(net, netPath, hosts, options.mode || undefined),
            ]));
            await Promise.all(promises).then(() => debug(`${net}: done`));
        }
        debug('done');
    },
    ...
    generateTincConfHook(net, netPath, hosts, mode = 'switch') {
        let output = '';
