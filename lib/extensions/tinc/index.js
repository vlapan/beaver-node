	...
	generate: function (callback) {
		...
		for (var i = 0, till = keys.length; i < till; i++) {
			...
			for (var j = 0, till2 = hostsKeys.length; j < till2; j++) {
				...
				fs.writeFileSync(path.normalize(hostsPath + '/' + key), hostFile, 'UTF-8');
			}

			function generateTincUpHook(item) {
				var output = '';
				output += '#!/bin/sh\n';
				output += 'ifconfig $INTERFACE ' + item.ip + ' netmask 255.255.255.0\n';
				output += 'ifconfig $INTERFACE up\n';
				output += 'ifconfig $INTERFACE name ' + item.name + '\n';
				output += '#route add -net 224.0.0.0/4 -interface $INTERFACE || echo \'route already exists\'\n';
