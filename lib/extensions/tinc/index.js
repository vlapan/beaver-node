...
const tinc = {
    async generate(o) {
        ...
        for (const [net, hosts] of Object.entries(nets)) {
            ...
            promisesHosts.push(Promise.all([
                ...
                tinc.copyRoutesScript(netPath, tap),
    ...
    copyRoutesScript(netPath, item) {
        return fs.copyFile(path.resolve(__dirname, 'routes.sh'), path.normalize(`${netPath}/routes.sh`));
    },
