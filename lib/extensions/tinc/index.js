	...
	generate: function (callback) {
		...
		for (var i = 0, till = keys.length; i < till; i++) {
			...
			function generateHostUpHook(item) {
				var output = '';
				output += '#!/bin/sh\n';
				output += 'logger -i -t "tinc.' + item.name + '" "host-up, name=$NAME, iface=$INTERFACE, node=$NODE, remoteaddress=$REMOTEADDRESS, remoteport=$REMOTEPORT, subnet=$SUBNET, weight=$WEIGHT"\n';
				fs.writeFileSync(path.normalize(netPath + '/host-up'), output, 'UTF-8');
			}
			function generateHostDownHook(item) {
				var output = '';
				output += '#!/bin/sh\n';
				output += 'logger -i -t "tinc.' + item.name + '" "host-down, name=$NAME, iface=$INTERFACE, node=$NODE, remoteaddress=$REMOTEADDRESS, remoteport=$REMOTEPORT, subnet=$SUBNET, weight=$WEIGHT"\n';
				fs.writeFileSync(path.normalize(netPath + '/host-down'), output, 'UTF-8');
			}
			function generateSubnetUpHook(item) {
				var output = '';
				output += '#!/bin/sh\n';
				output += 'logger -i -t "tinc.' + item.name + '" "subnet-up, name=$NAME, iface=$INTERFACE, node=$NODE, remoteaddress=$REMOTEADDRESS, remoteport=$REMOTEPORT, subnet=$SUBNET, weight=$WEIGHT"\n';
				fs.writeFileSync(path.normalize(netPath + '/subnet-up'), output, 'UTF-8');
			}
			function generateSubnetDownHook(item) {
				var output = '';
				output += '#!/bin/sh\n';
				output += 'logger -i -t "tinc.' + item.name + '" "subnet-down, name=$NAME, iface=$INTERFACE, node=$NODE, remoteaddress=$REMOTEADDRESS, remoteport=$REMOTEPORT, subnet=$SUBNET, weight=$WEIGHT"\n';
