...
module.exports = {
	generate: function (callback) {
		...
		Object.keys(config.servers).forEach(function (key, index) {
			if (typeof config.servers[key].router === 'string') {
		...
		for (var i = 0, till = keys.length; i < till; i++) {
			...
			for (var j = 0, till2 = hostsKeys.length; j < till2; j++) {
				...
				var vm = config.servers[key];
				var location = config.locations[vm.location];
				var address = key;
				logger.log('debug', '"' + key + '" host, net: ' + net + ', ip: ' + item.ip);

				var hostFile = '';
				if (!vm.wan && location.routing.external && vm.tcpShift) {
					var port = 3 + (vm.tcpShift | 0);
					[].concat(location.routing.external).forEach(function (address) {
