...
module.exports = {
	saveRootCert: function (certPath, callback) {
		...
		var ssl = config.routing.ssl;
		var prefixRoot = certPath + '/root-ca';
		Object.keys(ssl).forEach(function(key) {
			var item = ssl[key];
			fs.writeFileSync(prefixRoot + '-' + key + '.crt', item.rootCrt, 'UTF-8');
			fs.writeFileSync(prefixRoot + '-' + key + '.key', item.rootKey, 'UTF-8');
		});
		callback && callback();
	},
	generate: function (certPath, route, callback) {
		...
		var templateKey = typeof sslRoute === 'string' ? sslRoute : 'default';
		var ssl = config.routing.ssl[templateKey] || config.routing.ssl['default'];
		...
		exec(generate, function (error, stdout, stderr) {
			if (error) {
				logger.error('openssl generate:', error, stderr);
