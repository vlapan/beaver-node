    ...
    generate(certPath, route, callback) {
        ...
        if (Array.isArray(sslUse)) {
            ...
            for (let i = 0, till = sslUse.length; i < till; i++) {
                subjectSuffixDNS += ',DNS:' + sslUse[i] + ',DNS:*.' + sslUse[i];
            }
            if (typeof preset.subject === 'string') {
                ...
                subject = subjectPrefix.replace(/\/CN=$/, '/CN=' + route + '/ext:subjectAltName=DNS:' + route + subjectSuffixDNS);
            }
        } else {
            subject = (preset.subject || (subjectPrefix + route));
        }
