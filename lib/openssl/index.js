...
const { createSelfSigned, generateCertificate, generateCertificateFromCSR } = require('../utils');

const availableKeySizes = new Set([1024, 2048, 4096]);
const availableSignatureAlgorithms = new Set(['sha1', 'sha256', 'sha384', 'sha512']);
const acmePresets = new Set(['acme', 'acme-http-01', 'acme-dns-01']);
...
module.exports = {
    ...
    getKeySize(preset) {
        ...
        const keySize = Number.parseInt(preset.keySize, 10);
        return availableKeySizes.has(keySize) ? keySize : 4096;
    },
    ...
    getPreset(route) {
        ...
        const acmeAny = acmePresets.has(result.type);
    ...
    async signCSROld(csr, preset, prefix) {
        if (preset.acmeAny) {
            return false;
        }
        ...
        const signatureAlgorithm = preset.signatureAlgorithm && availableSignatureAlgorithms.has(preset.signatureAlgorithm) ? preset.signatureAlgorithm : 'sha256';
    ...
    async signCSR(csr, preset, prefix) {
        ...
        const expirationDays = Number.parseInt(preset.expirationDays, 10) || 365;
        const keySize = this.getKeySize(preset);
        const signatureAlgorithm = (preset.signatureAlgorithm && availableSignatureAlgorithms.has(preset.signatureAlgorithm) ? preset.signatureAlgorithm : 'sha256').replace(/^sha/i, 'SHA-');

        const cert = await generateCertificateFromCSR({
        ...
        await Promise.all([
            fs.promises.writeFile(`${prefix}.csr`, csr),
    ...
    async generate(conf, route) {
        ...
        if (typeof sslRoute === 'object' && sslRoute.crt && sslRoute.key) {
            if (sslRoute.crt.includes(', hash:') || sslRoute.key.includes(', hash:')) {
                return false;
            }
            await Promise.all([
                fs.promises.writeFile(`${prefix}.crt`, sslRoute.crt),
