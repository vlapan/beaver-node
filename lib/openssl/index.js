...
module.exports = {
	...
	generate: function (certPath, route, callback) {
		...
		var templateKey = typeof sslRoute === 'string' ? sslRoute : config.routing.ssl.defaultPreset;
		var ssl = config.routing.ssl.presets[templateKey] ? config.routing.ssl.presets[templateKey] : config.routing.ssl.presets[config.routing.ssl.defaultPreset];
