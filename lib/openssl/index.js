...
module.exports = {
	...
	generateRoot: function (certPath, callback) {
		...
		for (var i = 0, keys = Object.keys(presets), till = keys.length; i < till; i++) {
			...
			tasks.push((function (key, preset, callback) {
				var subTasks = [];
				if (preset.rootCA) {
					subTasks.push(fs.writeFile.bind(null, prefixRoot + '-' + key + '.ca', preset.rootCA, 'UTF-8'));
				}
				subTasks.push(fs.writeFile.bind(null, prefixRoot + '-' + key + '.crt', preset.rootCrt, 'UTF-8'));
				subTasks.push(fs.writeFile.bind(null, prefixRoot + '-' + key + '.key', preset.rootKey, 'UTF-8'));
				async.series(subTasks, function (err, result) {
					callback && callback(err);
				});
			}).bind(null, key, preset));
		}
