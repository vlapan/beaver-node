...
module.exports = {
    ...
    generate(certPath, route, callback) {
        ...
        if (Array.isArray(sslUse) && typeof subjectPrefix === 'string' && subjectPrefix.match(/\/CN=$/)) {
            let subjectSuffixCN = '';
            let subjectSuffixDNS = '';
            for (let i = 0, till = sslUse.length; i < till; i++) {
                subjectSuffixCN += '/' + (i + 1) + '.CN=' + sslUse[i];
                subjectSuffixDNS += ',DNS.' + (i + 1) + ':' + sslUse[i];
            }
            const subjectSuffix = '/0.CN=' + route + subjectSuffixCN + '/ext:subjectAltName=DNS.0:' + route + subjectSuffixDNS;
            subject = subjectPrefix.replace(/\/CN=$/, subjectSuffix);
        } else {
            subject = (preset.subject || (subjectPrefix + route)).replace(/\$\{([a-z0-9]+)\}/gi, (match, p1) => {
                switch (p1) {
                case 'CN':
                    return route;
                default:
