...
const { checkFileExists, checkDirectoryExists } = require('../utils/fs');
...
class Overseer {
    ...
    static async build(options) {
        ...
        if (typeof options.log === 'string') {
            ...
            if (await checkDirectoryExists(path.dirname(options.log))) {
                debug(`will log to '${options.log}'`);
                overseer.logStream = fs.createWriteStream(options.log, {
                    flags: 'a',
                });
            }
        }
    ...
    getSettingsString() {
        ...
        return `${this.config.tests.length} tests, ${modeName} mode, interval: ${this.config.interval} ms, tcpTimeout: ${this.config.tcpTimeout} ms, webTimeout: ${this.config.webTimeout} ms, maxAttempts: ${this.config.maxAttempts}`;
    }
    ...
    async processResults(data) {
        ...
        result.forEach((service) => {
            states.push(`${service.subject}, ${service.reason ? `${service.status}(${service.reason})` : service.status}, ${service.notify.join(' ')}`);
        });
    ...
    static colorLineByStatus(line, signs = ['+']) {
        return signs.includes(line[0]) ? (line.includes('FAIL') ? '700' : '070') : '999';
    }
    ...
    async generateNotification(current, previous) {
        ...
        diff.forEach((diffPart) => {
            ...
            parts.forEach((part) => {
                ...
                if (!diffPart.removed) {
                    const partWithPrefix = `+${part}`;
                    const partNoReason = partWithPrefix.replace(/\(.*\),/g, ',');
                    if (diffPart.added && diffTrigger.includes(partNoReason)) {
                        diffCurrent.push(partWithPrefix);
                        debug(partWithPrefix);
                        this.log(partWithPrefix);
                    } else {
                        diffCurrent.push(`~${part}`);
                    }
                }
                if (diffPart.removed) {
                    const partWithPrefix = `-${part}`;
                    const partNoReason = partWithPrefix.replace(/\(.*\),/g, ',');
                    if (diffTrigger.includes(partNoReason)) {
                        diffPreviously.push(partWithPrefix);
                        debug(partWithPrefix);
                        this.log(partWithPrefix);
                    }
                }
        ...
        for (const notifyKey of Object.keys(notify)) {
            ...
            if (diffPreviously.length > 0) {
                ...
                if (diffPreviouslyText.length > 0) {
                    ...
                    html += `<br><br><strong>PREVIOUSLY:</strong><br>${diffPreviouslyHtml.join('<br>')}`;
                }
            }
