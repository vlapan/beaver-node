...
Overseer.prototype.readConfig = function () {
    ...
    var tcpTimeout = this.config.tcpTimeout;
    this.config.tcp && this.config.tcp.forEach(function (service) {
        service.type = 'tcp';
        service.timeout = tcpTimeout;
        tests.push(service);
    });

    var webTimeout = this.config.webTimeout;
    this.config.web && this.config.web.forEach(function (service) {
        service.type = 'web';
...
Overseer.prototype.workerExitHandler = function (code, signal) {
...
Overseer.prototype.mail = function (to, subject, text, html) {
    var from = (process.env['USER'] || 'root') + '@' + os.hostname();
...
Overseer.prototype.getAddress = function (key) {
    var addresses = this.config.testing ? this.parseItem('testing') : this.parseItem(key);
