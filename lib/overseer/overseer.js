...
class Overseer {
    ...
    getSettingsString() {
        ...
        return `${modeName} mode, interval: ${this.config.interval} ms, tcpTimeout: ${this.config.tcpTimeout} ms, webTimeout: ${this.config.webTimeout} ms, maxAttempt: ${this.config.maxAttempt}`;
    }
    ...
    readConfig() {
        ...
        if (!this.config.maxAttempt) {
            debug(`maxAttempt not found, using default interval ${this.options.maxAttempt}`);
            this.config.maxAttempt = this.options.maxAttempt | 0;
        }
        ...
        if (this.config.tcp) {
            this.config.tcp.forEach((service) => {
                service.type = 'tcp';
                service.maxAttempt = this.config.maxAttempt;
