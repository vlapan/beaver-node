...
class Overseer {
    ...
    static mail(to, subject, text, html) {
        ...
        transporter.sendMail({
            ...
            response.statusHandler.once('failed', (data) => {
                debug(`Permanently failed delivering message to ${data.domain} with the following response: ${data.response}`);
            });

            response.statusHandler.once('requeue', (data) => {
                debug(`Temporarily failed delivering message to ${data.domain}`);
            });

            response.statusHandler.once('sent', (data) => {
                debug(`Message was accepted by ${data.domain}`);
            });
        });
    }
    ...
    generateNotification(current, previous) {
        ...
        Object.keys(notify).forEach((notifyKey) => {
            ...
            Overseer.mail(to, subject, text, html);
        });
    }
}
