        ...
        for (const notifyKey of Object.keys(notify)) {
            ...
            diffTrigger.forEach((line) => {
                ...
                if (list.includes(notifyKey)) {
                    text += `${line}\n`;
                    html += `<span style="color:#${line[0] === '+' ? (line.includes('FAIL') ? '700' : '070') : '999'};">${Overseer.makeOperationMonospaced(line)}</span><br>`;
                }
            });
            ...
            if (diffPreviously.length > 0) {
                ...
                diffPreviously.forEach((line) => {
                    ...
                    if (list.includes(notifyKey)) {
                        ...
                        diffPreviouslyHtml.push(`<span style="color:#${line.includes('FAIL') ? '700' : '070'};">${Overseer.makeOperationMonospaced(line)}</span>`);
                    }
                });
                if (diffPreviouslyText.length > 0) {
                    text += `\n\nPREVIOUSLY:\n${diffPreviouslyText}`;
                    html += `<br><br><strong>PREVIOUSLY:</strong><br>${diffPreviouslyHtml}`;
                }
            }
