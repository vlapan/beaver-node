...
const M = {
    ...
    parseKey(parser, k, a, n = true) {
        ...
        if (location) {
            ...
            if (net3) {
                if (net3.list) {
                    for (const item of net3.list) {
                        add(`${n === false ? item.ip : item.networkCidr}`, `${location.toString()}: NET3`);
                    }
                } else if (net3.networkCidr) {
                    add(`${n === false ? net3.ip : net3.networkCidr}`, `${location.toString()}: NET3`);
                }
            }
            ...
            if (wan36) {
                for (const item of [wan36].flat()) {
                    ...
                    add(`${n === false ? item.replace(/\/\d+$/, '') : (netmask ? item : `${item}/128`)}`, `${location.toString()}: WAN36`);
                }
            }
            ...
            if (lans) {
                if (lans.list) {
                    for (const item of lans.list) {
                        add(`${n === false ? item.ip : item.networkCidr}`, `${location.toString()}: LANS`);
                    }
                } else if (lans.networkCidr) {
                    add(`${n === false ? lans.ip : lans.networkCidr}`, `${location.toString()}: LANS`);
                }
            }
            return a;
        }
        ...
        if (server) {
            ...
            if (wan3) {
                add(`${wan3}${n === false ? '' : '/32'}`, `${server?.location?.toString() ?? '[Location(!UNDEF)]'}: ${server.toString()}: WAN3`);
            }
            const wan36 = server.wan36;
            if (wan36) {
                add(`${wan36}${n === false ? '' : '/128'}`, `${server?.location?.toString() ?? '[Location(!UNDEF)]'}: ${server.toString()}: WAN36`);
            }
            const lan3 = server.lan3;
            if (lan3) {
                add(`${lan3}${n === false ? '' : '/32'}`, `${server?.location?.toString() ?? '[Location(!UNDEF)]'}: ${server.toString()}: LAN3`);
            }
            return a;
        }
    ...
    redact: (data) => {
        for (const item of Object.values(data?.monitoring?.notify || {})) {
            if (item.auth &&  !item.auth.startsWith('[REDACTED:')) {
        ...
        for (const item of Object.values(data?.services?.git?.repositories || {})) {
            ...
            if (item.updateHookSecret && !item.updateHookSecret.startsWith('[REDACTED:')) {
    ...
    reductPKI: (data) => {
        for (const authority of Object.values(data?.services?.pki?.authorities || {})) {
            ...
            if (item.csr && !item.csr.startsWith('[REDACTED:')) {
