...
const M = {
    ...
    dateToString: (date) => {
        const epochMicros = BigInt(Date.now()) * 1000n + (process.hrtime.bigint() % 1000000n / 1000n);
        return `
            ${date.getUTCFullYear()}
            ...
            ${String(epochMicros % 1000000n).padStart(6, '0')}
        `.replace(/[\n ]+/g, '');
    },
    ...
    createSubject: (s) => {
        return new x509.Name(s).toJSON().map((x) => {
            const [key, value] = Object.entries(x).find(Boolean);
            return {
                [key]: [
                    {
                        [M.getAsn1CharStringType(key)]: value.at(0),
                    },
                ],
            };
        });
    ...
    generateCertificate: async ({
        ...
        issuerKeyCryptoKey.algorithm.hash = alg.hash;
        if (!keys) {
            keys = await crypto.subtle.generateKey(alg, false, ['sign', 'verify']);
        }
        const cert = await x509.X509CertificateGenerator.create({
            serialNumber: signatureAlgorithm === 'SHA-1' ? Date.now().toString(16) : M.dateToString(new Date()) + crypto.randomBytes(4).toString('hex') + dataHash,
            subject: M.createSubject(subject),
            issuer: issuerCrtObj.subjectName,
            notBefore,
            notAfter,
        ...
        return {
            key: keys.privateKey && crypto.KeyObject.from(keys.privateKey).export({
                type: 'pkcs8',
                format: 'pem',
            }),
            crt: [
    ...
    generateCertificateFromCSR: async (options) => {
        const csr = new x509.Pkcs10CertificateRequest(x509.PemConverter.decodeFirst(options.csr));
        const cn = csr.subject.split(', ').map((x) => x.match(/^CN=(.*)/)).find(Boolean);
        const san = csr.extensions.find((x) => x instanceof x509.SubjectAlternativeNameExtension)?.names?.items || (cn ? [{
            type: 'dns',
            value: cn[1],
        }] : false);
