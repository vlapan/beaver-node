...
config.readFile = async function () {
    ...
    try {
        ...
        const staticConfigsPath = path.resolve(`${argv.home}/conf.d`);
        const layer = {};
        if (await checkDirectoryExists(staticConfigsPath)) {
            const staticConfigsPaths = await findFile(staticConfigsPath, /.*\.json/, false);
            const staticConfigsPromises = staticConfigsPaths.sort().map((x) => fs.promises.readFile(x, 'utf8'));
            const staticConfigs = await Promise.all(staticConfigsPromises);
            for (const staticConfig of staticConfigs) {
                mergeObject(JSON.parse(staticConfig), layer);
            }
        }
        mergeObject(data, layer);
        await config.set(layer);
        config.failed = false;
    } catch (e) {
        config.failed = true;
        console.error(e);
