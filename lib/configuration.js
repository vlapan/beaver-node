...
config.parse = function () {
    ...
    logger.log('debug', '----------------------------------------------------------------------');
    Object.keys(config.routing.routes).map(routeKey => config.routing.routes[routeKey].target).reduce((a, b) => {
        if (!b) {
            return a;
        }
    ...
    if (argv.o) {
        const outputPath = config._outputPath = path.normalize(argv.o);
