...
config.parse = function () {
	...
	if (!hostConfig.location) {
		logger.error('Unable to find host\'s location "' + hostname + '" in config.vms');
		process.exit();
	}
	...
	config._hosts = {};
	for (var key of Object.keys(config.servers).concat(Object.keys(config.vms))) {
		if (config._hosts[key]) {
			return;
		}
		config._hosts[key] = config.servers[key] || config.vms[key];
	}
	...
	for (var i = 0, till = keys.length; i < till; i++) {
		var key = keys[i];
		config._routers.push(config.vms[key]);
	}
	...
	Object.keys(config.routing.routes).forEach(function (routeKey) {
		...
		[].concat(route.target).forEach(function (targetKey) {
...
var keys = ['locations', 'servers', 'test', 'vms', 'kvm', 'monitoring', 'routing'];
