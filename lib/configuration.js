...
config.loadClean = async function () {
    if (await checkFileExists(config.cleanPath)) {
        return fs.promises.readFile(config.cleanPath, 'utf8');
    }
    return '';
};

config.saveClean = async function () {
    await fs.promises.writeFile(config.cleanPath, config.parser.toSourceNonSecure());
    debugConfig(`non-secure version created`);
};
