...
function run(ssl) {
    ...
    function setAuthenticated(res, method) {
    ...
    debugWeb(`available authentication methods: ${Object.keys(authenticationMethods).join(', ')}`);
    debugWeb(`will attempt to enable the requested authentication methods: ${argv.authenticationMethods.join(', ')}`);
    ...
    for (const key of argv.authenticationMethods) {
        ...
        if (typeof method === 'function') {
            ...
            if (typeof result === 'string') {
                debugWeb(`${key}: load error: method failed to enable: "${result}"!`);
            } else {
                authenticationMethodsEnabled.push(key);
                debugWeb(`${key}: method was enabled successfully!`);
            }
        } else {
            debugWeb(`${key}: load error: method not found!`);
        }
    }
    debugWeb(`enabled authentication methods: ${authenticationMethodsEnabled.join(', ')}`);
