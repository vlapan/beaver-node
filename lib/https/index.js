...
if (fs.existsSync(`${sslPrefixConfigDir}`) && fs.existsSync(`${customSsl}.key`) && fs.existsSync(`${customSsl}.crt`)) {
    ...
    var key = fs.readFileSync(`${customSsl}.key`);
    var cert = fs.readFileSync(`${customSsl}.crt`);
...
function run(key, cert, ca) {
    ...
    for(let path of [
        '../../node_modules/clean-yaumnrc',
        '/usr/local/share/.config/yarn/global/node_modules/clean-yaumnrc']){
        if(fs.existsSync(path)) {
            app.use(
                '/clean-yaumnrc', 
                express.static(path[0] === '/' 
                    ? path 
