...
function run(ssl) {
    ...
    const checkBasicAuthData = (req, res, next) => {
        ...
        if (!basicData || !basicData.name || basicData.name === 'root' || basicData.name.length > 32 || !basicData.pass || basicData.pass.length > 128 || req.body.logout) {
            unauthorized(res);
            return;
        }
    ...
    const passwdAuth = (req, res, next) => {
        if (res.locals.authorized || !argv.authenticationMethods.includes('passwd')) {
            next();
            return;
        }
            ...
            if (err) {
                ...
                for (const item of arr) {
                    if (item.startsWith(`${basicData.name}:`)) {
                        ...
                        debugWeb(`"${basicData.name}" | ${passHash}`);
                        try {
                            if (M.unixcrypt.verify(basicData.pass, passHash)) {
                                // debugWeb(`"${basicData.name}" user authenticated!`);
                                authorized(res);
                                next();
                            } else {
    ...
    const pamAuth = (req, res, next) => {
        if (!pam || res.locals.authorized || !argv.authenticationMethods.includes('pam')) {
            next();
            return;
        }
        const basicData = res.locals.basicData;
