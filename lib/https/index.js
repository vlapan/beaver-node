...
if (fs.existsSync(`${sslPrefixConfigDir}`) && fs.existsSync(`${sslPrefixConfigDir}/${config._hostname}.key`) || fs.existsSync(`${sslPrefixConfigDir}/${config._hostname}.crt`)) {
  logger.log('info', `https: found https keys "${sslPrefixConfigDir}/${config._hostname}.*"`);
  var key = fs.readFileSync(`${sslPrefixConfigDir}/${config._hostname}.key`);
  var cert =  fs.readFileSync(`${sslPrefixConfigDir}/${config._hostname}.crt`);
  fs.readdir(`${sslPrefixConfigDir}`, function (err, files) {
    var ca = [];
    files.forEach(function (file) {
      if (file.indexOf('root-ca') === -1) {
        return;
      }
      ca.push(`${sslPrefixConfigDir}/${file}`);
    });
    run(key, cert, ca);
  });
  ...
  var key = fs.readFileSync(`${sslPrefix}.key`);
  var cert =  fs.readFileSync(`${sslPrefix}.crt`);
  var ca = fs.existsSync(`${argv.home}/root-ca.crt`) && fs.readFileSync(`${argv.home}/root-ca.crt`);
  run(key, cert, ca);
}
            ...
            tasks.push((callback) => {
              ...
              if (address) {
                ...
                if (vm.wan && vm.wan.ip) {
                  ...
                  host = [].concat(location.routing.external)[0];
              ...
              const options = {
                ...
                auth: auth.trim(),
                agent: false,
                ca: fs.existsSync(`${sslPrefix}.ca`) && fs.readFileSync(`${sslPrefix}.ca`),
