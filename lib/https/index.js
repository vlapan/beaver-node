...
function run(ssl) {
    ...
    const passwdAuth = (req, res, next) => {
        ...
        if (!basicData || res.locals.authorized || !argv.authenticationMethods.includes('passwd')) {
            next();
            return;
        }
    ...
    const pamAuth = (req, res, next) => {
        const basicData = res.locals.basicData;
        if (!basicData || !pam || res.locals.authorized || !argv.authenticationMethods.includes('pam')) {
            next();
            return;
        }
