				...
				if (req.body.forward && fs.existsSync(argv.home + '/bobot.auth')) {
					...
					forward.filter(function (key) {
						...
						tasks.push(function (callback) {
							//TODO: --cacert
							exec("curl 'https://" + config.vms[key].wan.ip + ":" + argv.httpsPort + "/' -u " + auth + " -H 'Content-Type: application/x-www-form-urlencoded' --data '" + querystring.stringify(configData).replace(/\'/gi, '%27') + "' --compressed --connect-timeout 2 --max-time 30 --silent --show-error --insecure", function (error, stdout, stderr) {
								if (stdout) {
