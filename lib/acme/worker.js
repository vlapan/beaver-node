...
async function retryable(cb, options = {}) {
    const opts = {
        attempt: 1,
        maxAttempt: 11,
        sleepMs: 144,
    ...
    try {
        ...
        debug(`retryable: ${Object.entries(opts).map((x) => `${x[0]}=${x[1]}`).join(', ')}`);
        console.log(e);
        if (opts.attempt < opts.maxAttempt) {
            await sleep(opts.sleepMs);
            opts.attempt += 1;
            opts.sleepMs = Math.round(opts.sleepMs * 1.618);
            return await retryable(cb, opts);
        } else {
...
async function challengeCreateFn(options, authz, challenge, keyAuthorization) {
    ...
    if (challenge.type === 'http-01') {
        ...
        const zoneFilePath = `${options.acmeDnsPath}/${dnsRecord}.dns`;
        ...
        await saveDNS(async () => {
            await fs.promises.writeFile(zoneFilePath, getZone(dnsRecord, options.dns.mname, options.dns.rname, options.dns.ns, records[dnsRecord]));
            await runHook(hookPath);
            await sleep(1000);
            debug(`check: NS servers(${options.dns.ns}) TXT records should include ${records[dnsRecord]}`);
            for (const ns of options.dns.ns) {
                ...
                for (const nsAddr of nsAddrList) {
                    ...
                    await retryable(async () => {
                        const txtArr = (await resolver.resolveTxt(dnsRecord)).map((x) => x.join(' '));
                        debug(`check:${ns}:${nsAddr}: resolved TXT: ${txtArr}`);
                        for (const record of records[dnsRecord]) {
                            const includes = txtArr.includes(record);
                            if (includes) {
                                continue;
                            }
                            debug(`check:${ns}:${nsAddr}: ${includes} === ${JSON.stringify(txtArr)}.includes(${JSON.stringify(record)})`);
                            throw new Error(`NS: ${ns}/${nsAddr}, no key(${record}) in TXT(${txtArr})`);
                        }
                    });
                }
            }
...
async function challengeRemoveFn(options, authz, challenge, keyAuthorization) {
    ...
    if (challenge.type === 'http-01') {
        ...
        await saveDNS(async () => {
            await fs.promises.writeFile(zoneFilePath, getZone(dnsRecord, options.dns.mname, options.dns.rname, [options.master], records[dnsRecord]));
            await runHook(hookPath);
            await sleep(1000);
        });
...
async function parse(config) {
    ...
    for (const [accountKey, account] of Object.entries(config.accounts)) {
        ...
        for (const domain of account.domains) {
            ...
            // if (domain.type === 'acme-dns-01') {
            //     debug(`'${domain.commonName}': skip dns`);
            //     continue;
            // }
