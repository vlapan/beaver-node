        ...
        await saveDNS(async () => {
            ...
            for (const ns of options.dns.ns) {
                ...
                for (const nsAddr of nsAddrList) {
                    ...
                    const txtArr = await resolver.resolveTxt(dnsRecord);
                    debug(`check:${ns}:${nsAddr}: resolved TXT: ${txtArr}`);
                    for (const record of records[dnsRecord]) {
                        const includes = txtArr.includes(record);
                        debug(`check:${ns}:${nsAddr}: ${JSON.stringify(txtArr)}.includes(${JSON.stringify(record)}) === ${includes}`);
                        if (includes) {
                            continue;
                        }
                        throw new Error(`dnsRecord: ${dnsRecord}, NS: ${ns}/${nsAddr}, no key(${record}) in TXT(${txtArr})`);
                    }
                }
            }
        });
