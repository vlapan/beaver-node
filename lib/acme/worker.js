...
const { getRemainingDays, checkMigrate, getZone } = require('./utils');
...
async function parse(config) {
    ...
    for (const [accountKey, account] of Object.entries(config.accounts)) {
        ...
        const [provider, type = 'production'] = account.provider.split('-');
        const directoryUrl = acme.directory?.[provider]?.[type] || config?.directory?.[provider]?.[type];
        if (directoryUrl === undefined) {
            const error = `Bad provider or type: ${account.provider}`;
            debug(`error: ${error}`);
            if (!errors[accountKey]) {
                errors[accountKey] = {};
            }
            errors[accountKey]['account settings'] = error;
            continue;
        }
        ...
        const client = new acme.Client({
            ...
            externalAccountBinding: account.externalAccountBinding,
        });
