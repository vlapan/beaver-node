...
const { sleep, retryable } = require('../utils');
        ...
        await saveDNS(async () => {
            ...
            for (const [key, value] of Object.entries(records)) {
                ...
                if (options.mirrorDomainSuffix) {
                    const zoneMirrorFilePath = `${options.acmeDnsPath}/${key}.${options.mirrorDomainSuffix}.dns`;
                    debug(`found mirrorDomainSuffix: ${options.mirrorDomainSuffix} ${zoneMirrorFilePath}`);
                    promises.push(fs.promises.writeFile(zoneMirrorFilePath, getZone(key, options.dns.mname, options.dns.rname, [options.master], value)));
                }
            }
