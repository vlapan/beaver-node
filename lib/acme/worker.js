...
async function challengeCreateFn(options, authz, challenge, keyAuthorization) {
    ...
    if (challenge.type === 'http-01') {
        ...
        await fs.promises.writeFile(zoneFilePath, getZone(dnsRecord, options.dns.mname, options.dns.rname, options.dns.ns, recordValue));
...
async function challengeRemoveFn(options, authz, challenge, keyAuthorization) {
    ...
    if (challenge.type === 'http-01') {
        ...
        await fs.promises.writeFile(zoneFilePath, getZone(dnsRecord, options.dns.mname, options.dns.rname, options.dns.ns));
        await runHook(hookPath);
    }
}
        ...
        for (const domain of account.domains) {
            ...
            try {
                const cert = await client.auto({
                    ...
                    email: account.email,
                    termsOfServiceAgreed: true,
                    challengePriority: domain.type === 'acme-dns-01' ? ['dns-01'] : ['http-01', 'dns-01'],
