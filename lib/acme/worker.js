...
async function challengeCreateFn(options, authz, challenge, keyAuthorization) {
    ...
    if (challenge.type === 'http-01') {
        ...
        const dnsRecord = `_acme-challenge.${authz.identifier.value}.dns`;
        const zoneFilePath = `${options.acmeDnsPath}/${dnsRecord}`;
        ...
        await fs.promises.writeFile(zoneFilePath, getZone(zone, options.dns.mname, options.dns.rname, options.dns.ns, recordValue));
...
async function challengeRemoveFn(options, authz, challenge, keyAuthorization) {
    ...
    if (challenge.type === 'http-01') {
        ...
        await fs.promises.writeFile(zoneFilePath, getZone(zone, options.dns.mname, options.dns.rname, options.dns.ns));
        await runHook(hookPath);
    }
}
