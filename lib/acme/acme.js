...
function Acme(options) {
    ...
    for (const [accountKey, account] of Object.entries(this.config.accounts)) {
        ...
        masterToPreset[account.master] = accountKey;
    ...
    this.masterToPreset = masterToPreset;
...
Acme.prototype.pullPresets = function () {
    async.series({
        ...
        for (const [master, preset] of Object.entries(this.masterToPreset)) {
            if (master === argv.hostname) {
                continue;
            }
            const accountPath = `${this.acmePath}/accounts/${preset}`;
            fs.readFile(`${accountPath}/serial`, 'utf-8', (err, data) => {
                this.pullPreset(results.ca, results.auth, master, preset, !err && data.split(' ')[0]);
            });
        }
    });
};
            ...
            https.request({
                ...
                if (res.statusCode === 200) {
                    ...
                    res.pipe(tar.extract({
                        ...
                        logger.log('info', `acme: pull preset: ${preset}: data error: ${err}`);
                        callback(true);
                    }).on('finish', () => {
                    ...
                    logger.log('info', `acme: pull preset: ${preset}: not found`);
                    callback(true);
                } else {
                    logger.log('info', `acme: pull preset: ${preset}: master returned code '${res.statusCode}'`);
                    callback(true);
                }
            }).on('timeout', function () {
