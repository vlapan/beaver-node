    ...
    async.series({
        ...
        presets.forEach((accountKey) => {
            ...
            fs.readFile(`${accountPath}/serial`, 'utf8', (err, data) => {
                ...
                async.each(hostnames, async (hostname, cb) => {
                    this.log('info', `push preset: ${accountKey}: pushing to ${hostname}`);
                    const stream = new Stream.PassThrough();
                    archiveStream.pipe(stream).pipe(await this.pushPreset(results.ca, results.auth, results.cookieJWT, hostname, accountKey, !err && data.split(' ')[0], cb));
                }, (err) => {
