...
function Acme(options) {
    ...
    http.createServer((req, res) => {
        if (this.config.active) {
            ...
            if (fs.existsSync(`/usr/local/www/acme/${req.url}`)) {
                res.setHeader('Content-Type', 'text/plain');
                res.end(fs.readFileSync(`/usr/local/www/acme/${req.url}`));
            } else {
                res.statusCode = 404;
                res.end();
            }
...
Acme.prototype.readConfig = function readConfig() {
    this.configChanged = false;
...
Acme.prototype.processResults = function processResults(data) {
    if (data.error) {
...
Acme.prototype.workerExitHandler = function workerExitHandler(code, signal) {
    logger.log('debug', 'acme: worker.js exit event', code, signal);
    this.active = false;
    this.watch();
};

Acme.prototype.start = function start() {
    if (this.active) {
        return;
    }
...
Acme.prototype.watch = function watch() {
