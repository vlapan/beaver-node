...
const { isPlainObject } = require('../utils/index');
...
function Acme(options) {
    ...
    http.createServer((req, res) => {
        if (!isPlainObject(this.hostToMaster)) {
            this.log('error', `http server: error: hostToMaster not found, probably there is no config yet`);
            res.statusCode = 500;
            res.end();
            return;
        }
...
Acme.prototype.processResults = function (data) {
    ...
    if (receivedEntries.length > 0 || errorsEntries.length > 0) {
        ...
        notificator.notify(message, 'daemon:acme');
    }
};
