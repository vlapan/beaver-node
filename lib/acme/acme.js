    ...
    http.createServer((req, res) => {
        ...
        if (!presetMaster) {
            ...
            http.request({
                ...
                this.log('error', 'http server: proxy: error: timeout');
                res.statusCode = 504;
                this.abort();
            }).on('error', (err) => {
                this.log('error', `http server: proxy: error: '${err}'`);
    ...
    async.series({
        ...
        request: (callback) => {
            ...
            https.request({
                ...
                this.log('error', `pull preset: ${preset}: error: timeout`);
                this.abort();
            }).on('error', (err) => {
...
Acme.prototype.pushPreset = function (ca, auth, hostname, preset, serial, callback) {
    return https.request({
        ...
        this.log('error', `push preset: ${preset}: error: timeout`);
        this.abort();
    }).on('error', (err) => {
...
Acme.prototype.processResults = function (data) {
    ...
    if (receivedEntries.length || errorsEntries.length) {
